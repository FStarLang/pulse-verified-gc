...
- **Latest proof step:** lifted all four concrete pre-collection edge cases into a quantified graph-edge morphism property. `GC.Gen.CombinedGraph` can recover minor/major field witnesses from `mem_ce` over `build_chunked_combined_graph`; `GC.Gen.CheneyCorrectness` maps ready reachable `MinorV -> MinorV`, reachable `MinorV -> MajorV`, old `MajorV -> MajorV`, and old `MajorV -> MinorV` edges to concrete post-collection major edges using `CG.fwd_morphism collect.cmc_fwd`. The readiness predicate is transparent and carries the old-major header witnesses plus raw-value update side conditions. `GC.SPOT.HeapExpansion` audits both the one-edge theorem and the quantified property.

Recent validation before this status update included targeted F* checks for `GC.Gen.CombinedGraph`, `GC.Gen.CheneyCorrectness`, and `GC.SPOT.HeapExpansion`; refreshed query-stat profiling showed the graph-edge wrappers verifying in the targeted `GC.Gen.CheneyCorrectness` profile in about 1.5 seconds overall and the new chunked vertex lemmas verifying in `GC.Gen.CombinedGraph` in about 1.1 seconds overall. Full SPOT plus broad `make generational` verification passed.

## Remaining steps

1. **Package graph-morphism readiness for clients.** Decide how to derive or bundle `chunked_graph_edge_maps_to_major_ready` from higher-level heap/root invariants so callers do not have to provide per-edge header/nonrewrite witnesses manually.
