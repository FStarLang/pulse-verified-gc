...
- **Latest proof step:** added a bundled forward graph-morphism surface. `GC.Gen.CheneyCorrectness` now maps ready reachable minor vertices through the forwarding map and old major vertices by identity to concrete post-collection major vertices, maps ready source edges to concrete post-collection major edges, and exposes `chunked_graph_maps_to_major_graph_prop` as the combined client-facing property. The edge readiness predicate remains transparent and carries the old-major header witnesses plus raw-value update side conditions. `GC.SPOT.HeapExpansion` audits the one-vertex, all-vertices, one-edge, all-edges, and bundled graph-morphism surfaces.

Recent validation before this status update included targeted F* checks for `GC.Gen.CombinedGraph`, `GC.Gen.CheneyCorrectness`, and `GC.SPOT.HeapExpansion`; refreshed query-stat profiling showed the graph-morphism wrappers verifying in the targeted `GC.Gen.CheneyCorrectness` profile in about 1.5 seconds overall and the chunked vertex lemmas verifying in `GC.Gen.CombinedGraph` in about 1.1 seconds overall. Full SPOT plus broad `make generational` verification passed.

## Remaining steps

1. **Package graph-morphism readiness for clients.** Decide how to derive or bundle `chunked_graph_vertex_maps_to_major_ready` and `chunked_graph_edge_maps_to_major_ready` from higher-level heap/root invariants so callers do not have to provide per-edge header/nonrewrite witnesses manually.
