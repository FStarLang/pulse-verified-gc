...
- **Latest proof step:** lifted both promoted-minor and old-major edge reasoning to graph-edge inputs. `GC.Gen.CombinedGraph` can recover minor/major field witnesses from `mem_ce` over `build_chunked_combined_graph`; `GC.Gen.CheneyCorrectness` maps actual pre-collection `MinorV -> MinorV` edges between reachable minor objects to post-collection `MajorV -> MajorV` forwarding-target edges, and now also maps old-major `MajorV -> MajorV` and `MajorV -> MinorV` graph edges through the chunked update path under the required raw-value rewrite side conditions. `GC.SPOT.HeapExpansion` audits these public contracts.

Recent validation before this status update included targeted F* checks for `GC.Gen.CombinedGraph`, `GC.Gen.CheneyCorrectness`, and `GC.SPOT.HeapExpansion`; refreshed query-stat profiling showed the new graph eliminator helpers verifying with local rlimit 5 and max used rlimit about 1.02, while the new old-major graph-edge wrappers verified in the targeted `GC.Gen.CheneyCorrectness` profile in about 1.5 seconds overall.
