...
- **Latest proof step:** discharged the remaining promoted-target field-slot witness from the public no-address successor-edge API. `GC.Gen.CheneyPreservation` now exposes a header-only forwarding-target theorem for ordinary promoted minor objects, `GC.Gen.CombinedGraph` turns active object header metadata into checked field-slot existence, and `GC.Gen.CheneyCorrectness` derives the copied payload slot internally instead of asking clients for either a concrete `field_addr` or an existential slot witness.

Recent validation before this status update included targeted F* checks for `GC.Gen.CheneyPreservation`, `GC.Gen.CheneyCorrectness`, and `GC.SPOT.HeapExpansion`; the new no-address wrappers remain audited through SPOT without the promoted-target slot witness.
