...
- **Latest proof step:** lifted the promoted-minor successor theorem to graph edges. `GC.Gen.CombinedGraph` can recover minor/major field witnesses from `mem_ce` over `build_chunked_combined_graph`, `GC.Gen.CheneyCorrectness` now maps an actual pre-collection `MinorV -> MinorV` edge between reachable minor objects to the post-collection `MajorV -> MajorV` edge between their forwarding targets, and `GC.SPOT.HeapExpansion` audits the public contracts.

Recent validation before this status update included targeted F* checks for `GC.Gen.CombinedGraph`, `GC.Gen.CheneyCorrectness`, and `GC.SPOT.HeapExpansion`; refreshed query-stat profiling showed the new graph eliminator helpers verifying with local rlimit 5 and max used rlimit about 1.02.
