...
- **Latest proof step:** lifted all four concrete pre-collection edge cases needed for the next graph-morphism theorem to graph-edge inputs. `GC.Gen.CombinedGraph` can recover minor/major field witnesses from `mem_ce` over `build_chunked_combined_graph`; `GC.Gen.CheneyCorrectness` maps reachable `MinorV -> MinorV` edges to forwarding-target `MajorV -> MajorV` edges, maps reachable `MinorV -> MajorV` edges to edges from the promoted source to the old target when the copied major value is not rewritten by the update guard, and maps old-major `MajorV -> MajorV` / `MajorV -> MinorV` edges through the chunked update path under the required raw-value rewrite side conditions. `GC.SPOT.HeapExpansion` audits these public contracts.

Recent validation before this status update included targeted F* checks for `GC.Gen.CombinedGraph`, `GC.Gen.CheneyCorrectness`, and `GC.SPOT.HeapExpansion`; refreshed query-stat profiling showed the graph-edge wrappers verifying in the targeted `GC.Gen.CheneyCorrectness` profile in about 1.5 seconds overall, and full SPOT plus broad `make generational` verification passed.

## Remaining steps

1. **Assemble chunked graph/successor preservation.** Lift the current per-edge mapping theorems into a graph-morphism-style theorem for the old-major plus reachable-minor subgraph, explicitly carrying the old-major raw-value aliasing/update side conditions and the `MinorV -> MajorV` nonrewrite condition.
