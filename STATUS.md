...
- **Latest proof step:** added pointwise graph-edge mapping for reachable minor successors, plus no-explicit-`field_addr` wrappers. The public no-address surface now consumes an existential promoted-target field-slot witness instead of requiring callers to pass the concrete copied payload address, avoiding a costly strengthening of the large Cheney preservation proof while still reducing the client-facing side condition. A follow-up `GC.Gen.CombinedGraph` bridge now proves that active major-object header metadata (`i < getWosize hdr`) implies the corresponding checked chunked field slot exists, isolating the non-Cheney arithmetic needed to discharge that witness later.

Recent validation before this status update included targeted F* checks for `GC.Gen.CombinedGraph`, `GC.Gen.CheneyCorrectness`, and `GC.SPOT.HeapExpansion`, a refreshed `GC.Gen.CheneyCorrectness` query-stats pass with the new no-address proofs at local rlimit 1, a `GC.Gen.CombinedGraph` query-stats pass with the new slot bridge at local rlimit 1, full SPOT verification, and broad `make generational`.
...
2. **Discharge the remaining slot witness.** The current no-address promoted-successor edge theorem no longer exposes the concrete copied payload address, but it still requires an existential proof that the promoted target has the `j`th major field slot. A cleaner final theorem should derive that witness from forwarding-target metadata and copied-object header facts, or first split/stabilize the heavy preservation module before adding the exact header/wosize invariant.
3. **Connect the graph theorem to the final end-to-end bundle.** Strengthen `GC.Gen.CheneyCorrectness` from pointwise edge consequences to a client theorem analogous to the fixed-heap end-to-end correctness properties, restated over `MH.major_heap` and `build_chunked_combined_graph`.
4. **Port or bridge major GC/mark-sweep surfaces.** The runtime expansion policy assumes major GC can run before expansion. The chunked allocator/expansion path is far along, but full chunk-aware major mark/sweep/coalesce remains a larger remaining stage unless kept behind a single-chunk compatibility boundary.
5. **Wire implementation/runtime acquisition.** Update the extracted/Pulse-facing allocation and collection entry points so they consume the chunked major heap representation, then update `alloc_gen.c` to allocate/register chunks, check alignment/disjointness/page-table facts, and call verified expansion/preflight functions.
6. **Extraction, docs, and integration tests.** Keep extraction stable, update design/runtime docs once the proof/API shape is final, and add integration scenarios where allocation or minor collection expands the major heap instead of failing.
