...
- **Latest proof step:** added vertex-image preservation alongside the quantified graph-edge morphism. `GC.Gen.CheneyCorrectness` now maps ready reachable minor vertices through the forwarding map and old major vertices by identity to concrete post-collection major vertices, and exposes a quantified vertex property. The edge readiness predicate remains transparent and carries the old-major header witnesses plus raw-value update side conditions. `GC.SPOT.HeapExpansion` audits the one-vertex, all-vertices, one-edge, and all-edges graph-morphism surfaces.
