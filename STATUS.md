...
Recent validation before this status update included targeted F* checks for `GC.Gen.CombinedGraph`, `GC.Gen.CheneyCorrectness`, and `GC.SPOT.HeapExpansion`, a refreshed `GC.Gen.CheneyCorrectness` query-stats pass with the new no-address proofs at local rlimit 1, full SPOT verification, and broad `make generational`.
