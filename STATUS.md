...
- **Latest proof step:** added a chunked minor-field classifier lemma and a new correctness corollary: if a reachable minor object has a field to a positive-size minor successor, the original chunked combined graph contains the `MinorV -> MinorV` edge and the preflight chunked collection forwards both endpoints. This is a small bridge toward full pre/post graph morphism preservation.

Recent validation before this status update included targeted F* checks for the changed `CombinedGraph` and `CheneyCorrectness` modules. A broad `make generational` refresh is currently running to refresh checked-file dependencies before rerunning the SPOT audit.

## Remaining steps

1. **Finish chunked graph/successor preservation.** Lift from field-level and old-object edge corollaries to a graph-morphism-style theorem for the chunked collection: old major edges remain valid, reachable minor edges map through forwarding, promoted minor object fields are represented in the post major graph, and root reachability is preserved after update and minor reset.
2. **Expose promoted-object field preservation after update.** The next hard proof surface is showing that copied minor fields in promoted chunked-major objects survive the later chunked major-pointer update in the right value-transformed form, enough to introduce post-collection `MajorV -> MajorV` edges for forwarded minor successors.
3. **Connect the graph theorem to the final end-to-end bundle.** Strengthen `GC.Gen.CheneyCorrectness` from local edge consequences to a client theorem analogous to the fixed-heap end-to-end correctness properties, restated over `MH.major_heap` and `build_chunked_combined_graph`.
