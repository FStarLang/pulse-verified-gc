...
- **Latest proof step:** added pointwise graph-edge mapping for reachable minor successors. If a reachable, scannable minor object has a field to a positive-size minor successor, the pre-collection chunked graph contains the `MinorV -> MinorV` edge, the preflight chunked collection forwards both endpoints, and the post-collection chunked graph contains the corresponding `MajorV -> MajorV` edge between their forwarding targets.

Recent validation before this status update included targeted F* checks for `GC.Gen.CheneyCorrectness` and `GC.SPOT.HeapExpansion`, a refreshed `GC.Gen.CheneyCorrectness` query-stats pass, full SPOT verification, and broad `make generational`.
...
2. **Reduce remaining edge theorem side conditions.** The current promoted-successor edge theorem still takes the copied promoted-field address explicitly. A cleaner client-facing theorem should derive that slot internally from forwarding-target metadata and copied-object header facts.
