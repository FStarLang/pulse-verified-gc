...
- **Latest proof step:** lifted all four concrete pre-collection edge cases into a generic one-edge morphism theorem. `GC.Gen.CombinedGraph` can recover minor/major field witnesses from `mem_ce` over `build_chunked_combined_graph`; `GC.Gen.CheneyCorrectness` maps reachable `MinorV -> MinorV`, reachable `MinorV -> MajorV`, old `MajorV -> MajorV`, and old `MajorV -> MinorV` edges to concrete post-collection major edges using `CG.fwd_morphism collect.cmc_fwd`, under an explicit transparent readiness predicate that carries the old-major header witnesses and raw-value update side conditions. `GC.SPOT.HeapExpansion` audits these public contracts.
...
1. **Quantify chunked graph/successor preservation.** Lift the generic one-edge theorem into a graph-morphism-style property for the old-major plus reachable-minor subgraph, and decide how to package/discharge the transparent readiness predicate for clients.
