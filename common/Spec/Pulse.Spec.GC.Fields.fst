...
/// Helper: Seq.mem ↔ List.Tot.mem via seq_to_list
let rec seq_mem_list_mem (#a:eqtype) (s: Seq.seq a) (x: a)
  : Lemma (ensures Seq.mem x s <==> List.Tot.mem x (Seq.seq_to_list s))
          (decreases Seq.length s)
  = if Seq.length s = 0 then ()
    else begin
      Seq.lemma_seq_of_list_induction (Seq.seq_to_list s);
      if Seq.head s = x then ()
      else seq_mem_list_mem (Seq.tail s) x
    end

/// Helper: Seq.index in seq_to_list implies List.Tot.mem
let seq_index_implies_list_mem (#a:eqtype) (s: Seq.seq a) (j: nat)
  : Lemma (requires j < Seq.length s)
          (ensures List.Tot.mem (Seq.index s j) (Seq.seq_to_list s))
  = // Seq.index s j is a member of s
    assert (Seq.mem (Seq.index s j) s);
    seq_mem_list_mem s (Seq.index s j)

/// When well_formed_heap holds, Seq.mem in objects implies List.Tot.mem in unpack_objects addresses
#push-options "--z3rlimit 400 --fuel 2 --ifuel 2"
let rec objects_mem_in_unpack_addrs (g: heap) (start: hp_addr) (x: obj_addr)
  : Lemma
    (requires well_formed_heap g /\ Seq.mem x (objects start g))
    (ensures List.Tot.mem x (List.Tot.map fst (Pulse.Spec.GC.Heap.unpack_objects g start)))
  ...
  = if U64.v start + 8 >= heap_size then ()
      ...
      if next_nat > heap_size || next_nat >= pow2 64 then ()
        ...
        getWosize_eq_header hdr;
        Pulse.Spec.GC.Heap.unpack_object_succeeds g start;
        Pulse.Spec.GC.Heap.unpack_object_addr g start;
        Pulse.Spec.GC.Heap.unpack_object_wz g start;
        if next_nat >= heap_size then begin
          Pulse.Spec.GC.Heap.unpack_objects_singleton g start;
          mem_cons_lemma x obj Seq.empty;
          ()
        end
        else begin
          ...
          let next : hp_addr = U64.uint_to_t next_nat in
          assert (objects start g == Seq.cons obj (objects next g));
          mem_cons_lemma x obj (objects next g);
          if x = obj then begin
            Pulse.Spec.GC.Heap.unpack_objects_head g start;
            ()
          end
          else begin
            assert (Seq.mem x (objects next g));
            objects_mem_in_unpack_addrs g next x;
            Pulse.Spec.GC.Heap.unpack_objects_mem_tail g start x
          end
        end
      end
    end
...
/// When a field value at index j is a pointer, exists_field_pointing_to_unchecked finds it.
#push-options "--z3rlimit 400 --fuel 2 --ifuel 1"
let field_value_implies_efptu (g: heap) (obj: obj_addr) (j: nat) (fv: U64.t)
  : Lemma
    (requires well_formed_object g obj /\
             U64.v (wosize_of_object obj g) < pow2 54 /\
             j < U64.v (wosize_of_object obj g) /\
             U64.v obj + FStar.Mul.(U64.v (wosize_of_object obj g) * 8) <= heap_size /\
             U64.v obj % 8 == 0 /\
             Some? (Pulse.Spec.GC.Heap.read_fields g obj (U64.v (wosize_of_object obj g)) 0) /\
             fv == Seq.index (Some?.v (Pulse.Spec.GC.Heap.read_fields g obj (U64.v (wosize_of_object obj g)) 0)) j /\
             is_pointer_field fv)
    (ensures exists_field_pointing_to_unchecked g obj (wosize_of_object obj g) fv)
  = let wz = wosize_of_object obj g in
    let wz_val = U64.v wz in
    Pulse.Spec.GC.Heap.read_fields_index g obj wz_val 0 j;
    let k : U64.t = U64.uint_to_t j in
    FStar.Math.Lemmas.pow2_lt_compat 61 54;
    FStar.Math.Lemmas.pow2_lt_compat 64 57;
    FStar.Math.Lemmas.modulo_lemma (FStar.Mul.(U64.v k * 8)) (pow2 64);
    FStar.Math.Lemmas.modulo_lemma (U64.v obj + FStar.Mul.(U64.v k * 8)) (pow2 64);
    field_read_implies_exists_pointing g obj wz k fv
#pop-options

/// Generic helper: prove List.Tot.for_all from a pointwise lemma
let rec for_all_true (#a:eqtype) (f: a -> bool) (l: list a)
  (pf: (x:a) -> Lemma (requires List.Tot.mem x l) (ensures f x = true))
  : Lemma (ensures List.Tot.for_all f l = true)
  = match l with
  | [] -> ()
  | hd :: tl ->
    pf hd;
...
/// Helper: for a single object that is a member of objects 0UL g, 
/// all pointer-like fields point to known addresses.
/// Returns the inner predicate result directly matching pointer_closed_ext's per-entry check.
#push-options "--z3rlimit 3000 --fuel 2 --ifuel 2"
let entry_fields_closed (g: heap) (start: hp_addr) (all_addrs: list obj_addr)
  : Lemma
    (requires well_formed_heap g /\
             all_addrs == List.Tot.map fst (Pulse.Spec.GC.Heap.unpack_objects g 0UL) /\
  ...
  = let (obj, ol) = Some?.v (Pulse.Spec.GC.Heap.unpack_object g start) in
    ...
    assert (Pulse.Spec.GC.Heap.hd_address obj == start);
    if U64.v ol.wz = 0 then ()
    else begin
      let wz_obj = wosize_of_object obj g in
      wosize_of_object_bound obj g;
      wf_object_size_bound g obj;
      let hdr_obj = read_word g (Pulse.Spec.GC.Heap.hd_address obj) in
      getWosize_eq_header hdr_obj;
      wosize_of_object_spec obj g;
      Pulse.Spec.GC.Heap.unpack_object_wz g start;
      Pulse.Spec.GC.Heap.unpack_object_fields g start;
      Pulse.Spec.GC.Heap.read_fields_succeeds g obj (U64.v wz_obj) 0;
      assert (U64.v ol.wz == U64.v wz_obj);
      assert (ol.fields == Some?.v (Pulse.Spec.GC.Heap.read_fields g obj (U64.v wz_obj) 0));
      let inner_pred (fv: U64.t) : bool =
        if U64.v fv >= 8 && U64.v fv < heap_size && U64.v fv % 8 = 0
        then List.Tot.mem fv all_addrs
        else true
      in
      let flds = Seq.seq_to_list ol.fields in
      let field_check (fv: U64.t) : Lemma
        (requires List.Tot.mem fv flds /\ U64.v fv >= 8 /\ U64.v fv < heap_size /\ U64.v fv % 8 = 0)
        (ensures List.Tot.mem fv all_addrs)
        = seq_mem_list_mem ol.fields fv;
          let j = Seq.index_mem fv ol.fields in
          field_value_implies_efptu g obj j fv;
          objects_mem_in_unpack_addrs g 0UL fv
      in
      for_all_true inner_pred flds (fun fv -> 
        if U64.v fv >= 8 && U64.v fv < heap_size && U64.v fv % 8 = 0
        then field_check fv
        else ())
    end
...
/// Helper: members of objects at a later start are members of objects 0UL g,
/// given that the earlier start's objects are contained in objects 0UL g
let objects_suffix_in_global (g: heap) (start: hp_addr) (obj: obj_addr) (next: hp_addr) (x: obj_addr) 
  : Lemma
    (requires Seq.length (objects start g) > 0 /\
             ...
             (forall (y: obj_addr). Seq.mem y (objects start g) ==> Seq.mem y (objects 0UL g)) /\
             Seq.mem x (objects next g))
    (ensures Seq.mem x (objects 0UL g))
  = objects_nonempty_next start g;
    mem_cons_lemma x obj (objects next g)

/// When unpack_object succeeds with size conditions, objects is nonempty
#push-options "--z3rlimit 100 --fuel 1 --ifuel 1"
let unpack_implies_objects_nonempty (g: heap) (h: hp_addr) : Lemma
  (requires U64.v h + 8 < heap_size /\ Some? (Pulse.Spec.GC.Heap.unpack_object g h) /\
            (let wz = (snd (Some?.v (Pulse.Spec.GC.Heap.unpack_object g h))).Pulse.Spec.GC.Heap.wz in
             let next_nat = U64.v h + FStar.Mul.((U64.v wz + 1) * 8) in
             next_nat <= heap_size /\ next_nat < pow2 64))
  (ensures Seq.length (objects h g) > 0)
  = Pulse.Spec.GC.Heap.unpack_object_wz g h;
    let hdr = read_word g h in
    getWosize_eq_header hdr;
    ()
#pop-options

/// Reverse direction: List.Tot.mem in unpack_objects addresses implies Seq.mem in objects
#push-options "--z3rlimit 800 --fuel 2 --ifuel 2"
let rec unpack_addrs_in_objects_mem (g: heap) (start: hp_addr) (x: obj_addr)
  : Lemma
    (requires well_formed_heap g /\ List.Tot.mem x (List.Tot.map fst (Pulse.Spec.GC.Heap.unpack_objects g start)))
    (ensures Seq.mem x (objects start g))
    (decreases (heap_size - U64.v start))
  = if U64.v start + 8 >= heap_size then
      Pulse.Spec.GC.Heap.unpack_objects_empty_start g start
    else begin
      let hdr = read_word g start in
      let wz = getWosize hdr in
      let obj = obj_address start in
      ...
      match Pulse.Spec.GC.Heap.unpack_object g start with
      | None -> Pulse.Spec.GC.Heap.unpack_objects_empty_overflow g start
      | Some (uobj, ol) ->
        Pulse.Spec.GC.Heap.unpack_object_addr g start;
        Pulse.Spec.GC.Heap.unpack_object_wz g start;
        ...
        if next_nat > heap_size || next_nat >= pow2 64 then
          Pulse.Spec.GC.Heap.unpack_objects_empty_overflow g start
        else if next_nat >= heap_size then begin
          Pulse.Spec.GC.Heap.unpack_objects_singleton g start;
          objects_head_mem start g
        end
        else begin
          let next : hp_addr = U64.uint_to_t next_nat in
          Pulse.Spec.GC.Heap.unpack_objects_cons g start;
          if x = obj then
            objects_head_mem start g
          else begin
            unpack_addrs_in_objects_mem g next x;
            objects_nonempty_next start g;
            mem_cons_lemma x obj (objects next g)
          end
        end
...
/// Prove universal entry_check_at (non-recursive), then use structural proof from Heap.fst
#push-options "--z3rlimit 3000 --fuel 2 --ifuel 2"
let wfh_pointer_closed_direct (g: heap) : Lemma
  (requires well_formed_heap g)
  (ensures Pulse.Spec.GC.Heap.pointer_closed (Pulse.Spec.GC.Heap.unpack_objects g 0UL) = true)
  = let addrs = List.Tot.map fst (Pulse.Spec.GC.Heap.unpack_objects g zero_addr) in
    // Prove entry_check_at for each h
    let aux (h: hp_addr) : Lemma
      (requires U64.v h + 8 < heap_size /\
               (match Pulse.Spec.GC.Heap.unpack_object g h with
                | None -> True
                | Some (_, ol) ->
                  let next_nat = U64.v h + FStar.Mul.((U64.v ol.Pulse.Spec.GC.Heap.wz + 1) * 8) in
                  next_nat <= heap_size /\ next_nat < pow2 64 /\
                  List.Tot.mem (U64.uint_to_t (U64.v h + 8)) addrs))
      (ensures Pulse.Spec.GC.Heap.entry_check_at g h addrs = true)
    = match Pulse.Spec.GC.Heap.unpack_object g h with
      | None -> ()
      | Some (obj, ol) ->
    ...
    in
    FStar.Classical.forall_intro (FStar.Classical.move_requires aux);
    Pulse.Spec.GC.Heap.pointer_closed_from_universal_0 g addrs
#pop-options
...
let wfh_implies_unpack (g: heap) : Lemma
  ...
  = wfh_pointer_closed_direct g;
    Pulse.Spec.GC.Heap.pointer_closed_implies_unpack g
#pop-options
