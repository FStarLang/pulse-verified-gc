...
krml_checked_int_t fwd_array_size;

static void rewrite_roots_impl(uint64_t *roots, uint64_t *fwd_arr, size_t n)
{
  size_t i = (size_t)0U;
  size_t __anf0 = i;
  bool cond = __anf0 < n;
...
{
  /* PATCH for OCaml integration: lower-bound check for NULL-base trick */
  if (v < zero_addr + 8ULL)
    return false;
...
/* PATCH for OCaml integration: non-static, called by bridge for root darkening */
...
{
  size_t base = (size_t)addr;
  uint8_t b0 = mh.data[base];
  uint8_t b1 = mh.data[base + (size_t)1U];
  uint8_t b2 = mh.data[base + (size_t)2U];
  uint8_t b3 = mh.data[base + (size_t)3U];
  uint8_t b4 = mh.data[base + (size_t)4U];
  uint8_t b5 = mh.data[base + (size_t)5U];
  uint8_t b6 = mh.data[base + (size_t)6U];
  uint8_t b7 = mh.data[base + (size_t)7U];
  return
    (uint64_t)b0 | (uint64_t)b1 << 8U | (uint64_t)b2 << 16U | (uint64_t)b3 << 24U |
      (uint64_t)b4 << 32U
    | (uint64_t)b5 << 40U
    | (uint64_t)b6 << 48U
    | (uint64_t)b7 << 56U;
...
{
  size_t base = (size_t)addr;
  uint8_t b0 = (uint8_t)v;
  uint8_t b1 = (uint8_t)(v >> 8U);
  uint8_t b2 = (uint8_t)(v >> 16U);
  uint8_t b3 = (uint8_t)(v >> 24U);
  uint8_t b4 = (uint8_t)(v >> 32U);
  uint8_t b5 = (uint8_t)(v >> 40U);
  uint8_t b6 = (uint8_t)(v >> 48U);
  uint8_t b7 = (uint8_t)(v >> 56U);
  mh.data[base] = b0;
...
{
  /* PATCH for OCaml integration: use zero_addr for NULL-base trick */
  return v >= zero_addr + 8ULL && v < GC_Spec_Base_heap_size_u64 && v % 8ULL == 0ULL;
}
