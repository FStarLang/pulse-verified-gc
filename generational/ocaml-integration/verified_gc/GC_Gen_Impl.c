...
/* Bridge-visible: set by alloc_gen.c to heap base absolute address */
uint64_t zero_addr = 0ULL;

/* High-water mark for major heap allocation (scan optimization) */
uint64_t major_alloc_hwm = 0ULL;

/* If > 0, update_all_objects starts scanning from this address */
...
static uint64_t fused_sweep_coalesce(heap_t heap)
{
  ...
  bool cond = __anf0 + 8ULL < heap_size_u64;
  while (cond)
  {
    ...
    uint64_t __anf0 = current;
    cond = __anf0 + 8ULL < heap_size_u64;
  }
...
{
  uint64_t total_words = heap_size_u64 / 8ULL;
  if (total_words < 2ULL)
    return 0ULL;
  else
  {
...
K___uint64_t_uint64_t allocate(heap_t heap, uint64_t fp, uint64_t wosize)
{
  ...
  bool go = true;
  uint64_t fuel_ref = heap_size_u64 / 8ULL;
  while (go)
  {
