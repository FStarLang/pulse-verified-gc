...
#push-options "--z3rlimit 10 --fuel 1 --ifuel 1 --split_queries always"
private let rec chunked_fl_chain_avoids_transfer
  ...
  if fuel = 0 then ()
    ...
    | Some hdr ->
      ...
      | Some next ->
        ...
        chunked_fl_chain_avoids_transfer mh0 mh1 excl next fuel';
        assert
          (match MH.read_word_in_major mh1 (cur <: obj_addr) with
           | Some next' ->
  ...
  if fuel = 0 then ()
    ...
    | Some hdr ->
      ...
      | Some next ->
        ...
        assert (SpecMajorAlloc.major_fl_chain_avoids
                  mh0 next excl fuel' = true);
        let tail_link_frame (src: obj_addr)
          : Lemma
              (requires
                Seq.mem src (MH.major_objects mh0) /\
                src <> excl /\
        ...
        FStar.Classical.forall_intro
          (FStar.Classical.move_requires tail_link_frame);
...
private let chunked_chain_objects_blue_head_is_blue_for_transfer
...
private let chunked_chain_objects_blue_zero_for_transfer
  (mh: MH.major_heap) (fp: U64.t)
...
private let chunked_chain_objects_blue_null_for_transfer
...
private let chunked_chain_objects_blue_tail_for_transfer
  (mh: MH.major_heap) (fp: U64.t) (fuel: nat) (next: U64.t)
  : Lemma
      (requires
        fuel > 0 /\
        fp <> 0UL /\
        ...
        GenInv.chunked_chain_objects_blue mh fp fuel /\
        MH.read_word_in_major mh (fp <: obj_addr) == Some next)
      (ensures GenInv.chunked_chain_objects_blue mh next (fuel - 1))
  =
  SpecMajorAlloc.major_fl_above_zero_current mh fp fuel;
  assert (U64.v fp >= U64.v zero_addr + U64.v mword);
  assert (U64.v fp >= U64.v mword);
  ...
  let tail (obj: obj_addr)
    ...
    if obj = fp_obj then begin
      ...
      assert (fp <> obj);
      SpecMajorAlloc.major_fl_chain_avoids_tail mh fp obj fuel
    end
  in
  FStar.Classical.forall_intro (FStar.Classical.move_requires tail);
  GenInv.chunked_chain_objects_blue_intro mh next (fuel - 1)

private let chunked_chain_objects_blue_step_blue_head_for_transfer
  (mh: MH.major_heap) (fp: U64.t) (fuel: nat)
  (hdr next: U64.t)
  : Lemma
      (requires
        ...
        U64.v fp < heap_size /\
        U64.v fp % U64.v mword == 0 /\
        MH.read_word_in_major mh (hd_address (fp <: obj_addr)) == Some hdr /\
        getColor hdr = Blue /\
        MH.read_word_in_major mh (fp <: obj_addr) == Some next /\
        GenInv.chunked_chain_objects_blue mh next (fuel - 1))
      (ensures GenInv.chunked_chain_objects_blue mh fp fuel)
  =
  let fp_obj : obj_addr = fp in
  let aux (obj: obj_addr)
    : Lemma
    ...
    =
    if obj = fp_obj then begin
      GenInv.chunked_is_blue_header mh fp_obj hdr;
      assert (GenInv.chunked_is_blue mh obj);
      assert False
    end else begin
      assert (fp <> obj);
      GenInv.chunked_chain_objects_blue_elim mh next (fuel - 1) obj;
      SpecMajorAlloc.major_fl_chain_avoids_step mh fp obj fuel
    end
  in
  FStar.Classical.forall_intro (FStar.Classical.move_requires aux);
  GenInv.chunked_chain_objects_blue_intro mh fp fuel

private let rec chunked_chain_objects_blue_transfer_on_chain
  (mh0 mh1: MH.major_heap) (dst cur: U64.t) (fuel: nat)
  : Lemma
      (requires
        SpecMajorAlloc.major_fl_valid mh0 cur fuel /\
        SpecMajorAlloc.major_fl_above_zero mh0 cur fuel /\
        SpecMajorAlloc.major_fl_chain_avoids mh0 cur dst fuel = true /\
        GenInv.chunked_chain_objects_blue mh0 cur fuel /\
        (forall (src: obj_addr).
          Seq.mem src (MH.major_objects mh0) /\
          src <> dst /\
          (match MH.read_word_in_major mh0 (hd_address src) with
           | Some hdr -> U64.v (getWosize hdr) >= 1
           | None -> False) /\
          (match MH.read_word_in_major mh0 src with
           | Some _ -> True
           | None -> False) /\
          SpecMajorAlloc.major_fl_chain_avoids mh0 cur src fuel = false ==>
          MH.read_word_in_major mh1 (hd_address src) ==
          ...
          MH.read_word_in_major mh1 src ==
          MH.read_word_in_major mh0 src))
      (ensures GenInv.chunked_chain_objects_blue mh1 cur fuel)
      (decreases fuel)
  =
  if fuel = 0 then
    chunked_chain_objects_blue_zero_for_transfer mh1 cur
  else if cur = 0UL then
    chunked_chain_objects_blue_null_for_transfer mh1 fuel
  else begin
    assert (fuel > 0);
    let fuel' : f:nat{f < fuel} = fuel - 1 in
    SpecMajorAlloc.major_fl_above_zero_current mh0 cur fuel;
    ...
    SpecMajorAlloc.major_fl_valid_next mh0 cur fuel;
    SpecMajorAlloc.major_fl_chain_avoids_tail mh0 cur dst fuel;
    chunked_chain_objects_blue_head_is_blue_for_transfer mh0 cur fuel;
    match MH.read_word_in_major mh0 (hd_address x) with
    | None -> assert False
    | Some hdr ->
      assert (U64.v (getWosize hdr) >= 1);
      GenInv.chunked_is_blue_header mh0 x hdr;
      ...
      | Some next ->
        ...
        assert (SpecMajorAlloc.major_fl_chain_avoids
                  mh0 next dst fuel' = true);
        assert (SpecMajorAlloc.major_fl_chain_avoids
                  mh0 cur x fuel = false);
        assert (MH.read_word_in_major mh1 (hd_address x) == Some hdr);
        assert (MH.read_word_in_major mh1 x == Some next);
        chunked_chain_objects_blue_tail_for_transfer mh0 cur fuel next;
        ...
        chunked_chain_objects_blue_transfer_on_chain
          mh0 mh1 dst next fuel';
        chunked_chain_objects_blue_step_blue_head_for_transfer
          mh1 cur fuel hdr next
  end
#pop-options
...
private let major_alloc_head_split_preserves_chain_objects_blue
  ...
  : Lemma
      ...
      (ensures
        (let r =
           SpecMajorAlloc.major_alloc_spec_with_fuel mh fp wosize fuel in
         GenInv.chunked_chain_objects_blue r.major_alloc_out r.major_fp_out fuel))
  =
  GenInv.chunked_major_alloc_shape_elim mh fp fuel;
  SpecMajorAlloc.major_fl_above_zero_current mh fp fuel;
  assert (U64.v fp >= U64.v zero_addr + U64.v mword);
  ...
  | Some old_hdr ->
    ...
    | Some next_fp ->
      ...
      SpecMajorAllocSplitShape.major_alloc_head_split_preserves_alloc_shape
        mh fp wosize fuel;
      let tail_fuel : f:nat{f < fuel} = fuel - 1 in
      SpecMajorAlloc.major_fl_chain_predecessor_not_in_suffix mh fp fuel;
      assert (SpecMajorAlloc.major_fl_chain_avoids mh next_fp fp tail_fuel = true);
      SpecMajorAlloc.major_fl_valid_next mh fp fuel;
      SpecMajorAlloc.major_fl_above_zero_next mh dst fuel next_fp;
      SpecMajorAlloc.major_fl_blocks_fit_next mh dst fuel next_fp;
      assert (SpecMajorAlloc.major_fl_valid mh next_fp tail_fuel);
      assert (SpecMajorAlloc.major_fl_above_zero mh next_fp tail_fuel);
      assert (SpecMajorAlloc.major_fl_blocks_fit mh next_fp tail_fuel);
      chunked_chain_objects_blue_tail_for_transfer mh fp fuel next_fp;
      let split_chain_frame (src: obj_addr)
        : Lemma
            (requires
              Seq.mem src (MH.major_objects mh) /\
              src <> fp /\
              ...
              (match MH.read_word_in_major mh src with
               ...
               | None -> False) /\
              SpecMajorAlloc.major_fl_chain_avoids
                mh next_fp src tail_fuel = false)
            (ensures
              MH.read_word_in_major alloc_res.major_alloc_out (hd_address src) ==
        ...
        | Some src_hdr ->
          ...
          | Some src_next ->
            assert (src <> dst);
            MH.read_word_in_major_lookup_index mh (hd_address src) src_hdr;
            let src_idx = MH.lookup_chunk_index_value mh (hd_address src) in
            assert (MH.lookup_chunk_index mh (hd_address src) == Some src_idx);
            assert (src_idx < Seq.length mh);
            assert (MH.word_in_chunk (Seq.index mh src_idx) (hd_address src));
            MH.major_objects_member_in_lookup_chunk mh src_idx src;
            MH.objects_in_chunk_member_header_fits (Seq.index mh src_idx) src;
            assert (MH.object_header_size_fits_in_chunk
                      (Seq.index mh src_idx) src);
            assert (U64.v (hd_address src) +
                    (1 + U64.v (getWosize src_hdr)) * U64.v mword <=
                    MH.chunk_end (Seq.index mh src_idx));
            SpecMajorAllocSplitShape.head_split_preserves_non_head_node_facts
              mh idx dst src src_hdr src_next wosize block_wz next_fp
              rem_wz_u rem_hd rem_obj
      in
      FStar.Classical.forall_intro
        (FStar.Classical.move_requires split_chain_frame);
      chunked_chain_objects_blue_transfer_on_chain
        mh alloc_res.major_alloc_out fp next_fp tail_fuel;
      SpecMajorAllocSplitShape.head_split_remainder_head_facts
        mh idx dst wosize block_wz next_fp rem_wz_u rem_hd rem_obj;
      let rem_hdr = SpecAlloc.make_header rem_wz_u SpecAlloc.blue_bits 0UL in
      assert (MH.read_word_in_major alloc_res.major_alloc_out rem_hd == Some rem_hdr);
      assert (MH.read_word_in_major alloc_res.major_alloc_out rem_obj == Some next_fp);
      ...
      AllocLemmas.make_header_getColor rem_wz_u SpecAlloc.blue_bits 0UL;
      assert (getColor rem_hdr = Blue);
      assert (U64.v rem_obj >= U64.v mword);
      assert (U64.v rem_obj < heap_size);
      assert (U64.v rem_obj % U64.v mword == 0);
      chunked_chain_objects_blue_step_blue_head_for_transfer
        alloc_res.major_alloc_out rem_obj fuel rem_hdr next_fp;
  ...
  | Some old_hdr ->
    ...
    | Some next_fp ->
      ...
      assert (MH.major_objects res.major_out ==
              MH.major_objects alloc_res.major_alloc_out);
      let old_member (src: obj_addr)
        : Lemma
            (requires Seq.mem src (MH.major_objects mh))
            (ensures Seq.mem src (MH.major_objects res.major_out))
        =
        MH.major_objects_member_header_read_some mh src;
        match MH.read_word_in_major mh (hd_address src) with
        | None -> assert False
        | Some old_src_hdr ->
          SpecMajorAlloc.head_split_preserves_free_node_object
            mh idx dst src old_src_hdr wosize block_wz next_fp
            rem_wz_u rem_hd rem_obj;
          assert (Seq.mem src (MH.major_objects alloc_res.major_alloc_out))
      in
      FStar.Classical.forall_intro
        (FStar.Classical.move_requires old_member);
      ...
      let old_header_frame (src: obj_addr) (hdr: U64.t)
        : Lemma
            (requires Seq.mem src (MH.major_objects mh) /\
                      ...
                      MH.read_word_in_major mh (hd_address src) ==
                        Some hdr /\
                      U64.v (getWosize hdr) >= 1)
            (ensures MH.read_word_in_major res.major_out (hd_address src) ==
                     Some hdr)
        =
        let src_hd = hd_address src in
        ...
        | Some old_next ->
          ...
          assert (MH.read_word_in_major final_major src_hd ==
                  MH.read_word_in_major alloc_res.major_alloc_out src_hd)
      in
      let old_header_frame_for_src (src: obj_addr)
        : Lemma
            (ensures forall (hdr: U64.t).
              Seq.mem src (MH.major_objects mh) /\
              src <> dst /\
              MH.read_word_in_major mh (hd_address src) == Some hdr /\
              U64.v (getWosize hdr) >= 1 ==>
        ...
        =
        FStar.Classical.forall_intro
          (FStar.Classical.move_requires (old_header_frame src))
      in
      FStar.Classical.forall_intro old_header_frame_for_src;
...
#push-options "--z3rlimit 10 --fuel 0 --ifuel 0 --split_queries always"
let chunked_promote_object_head_split_preserves_chain_objects_blue
  (minor: minor_state) (mh: MH.major_heap) (obj: U64.t)
  (fp: U64.t) (wosize: nat{wosize > 0}) (fuel: nat)
  : Lemma
      (requires
  ...
  chunked_promote_object_head_split_preserves_chunked_alloc_shape
    minor mh obj fp wosize fuel;
  major_alloc_head_split_preserves_chain_objects_blue mh fp wosize fuel;
  GenInv.chunked_major_alloc_shape_elim mh fp fuel;
  SpecMajorAllocSplitShape.major_alloc_head_split_preserves_alloc_shape
    mh fp wosize fuel;
  SpecMajorAllocSplitShape.major_alloc_head_split_remainder_avoids_allocated_head
    ...
    : Lemma
        (requires
          ...
          (match MH.read_word_in_major
           ...
           | Some hdr -> U64.v (getWosize hdr) >= 1
           | None -> False) /\
          (match MH.read_word_in_major alloc_res.major_alloc_out src
           with
           | Some _ -> True
           | None -> False) /\
          SpecMajorAlloc.major_fl_chain_avoids
  ...
  chunked_chain_objects_blue_transfer_on_chain
    alloc_res.major_alloc_out res.major_out fp alloc_res.major_fp_out fuel;
  assert (GenInv.chunked_chain_objects_blue
            res.major_out alloc_res.major_fp_out fuel)
#pop-options

#push-options "--z3rlimit 10 --fuel 0 --ifuel 0 --split_queries always"
let chunked_promote_object_head_split_preserves_old_non_blue_header
  (minor: minor_state) (mh: MH.major_heap) (obj: U64.t)
  (fp: U64.t) (wosize: nat{wosize > 0}) (fuel: nat)
...
#push-options "--z3rlimit 10 --fuel 1 --ifuel 0 --split_queries always"
[@@"opaque_to_smt"]
let chunked_fwd_targets_above_minor (fwd: forwarding_map) : prop =
  forall (x: U64.t).
    fwd x <> 0UL ==>
    U64.v (fwd x) >= minor_heap_size /\
    U64.v (fwd x) % U64.v mword == 0

let chunked_fwd_targets_above_minor_empty ()
  : Lemma (ensures chunked_fwd_targets_above_minor empty_forwarding)
  =
  reveal_opaque (`%chunked_fwd_targets_above_minor)
    (chunked_fwd_targets_above_minor empty_forwarding);
  let aux (x: U64.t)
    : Lemma
        (requires empty_forwarding x <> 0UL)
        (ensures U64.v (empty_forwarding x) >= minor_heap_size /\
                U64.v (empty_forwarding x) % U64.v mword == 0)
    =
...
let chunked_fwd_targets_above_minor_elim
  ...
  =
  reveal_opaque (`%chunked_fwd_targets_above_minor)
    (chunked_fwd_targets_above_minor fwd)

private let chunked_fwd_target_above_minor_from_major_addr
  (v: U64.t)
  : Lemma
      (requires U64.v v >= U64.v zero_addr + U64.v mword /\
               U64.v v % U64.v mword == 0)
      (ensures U64.v v >= minor_heap_size /\
              U64.v v % U64.v mword == 0)
  =
...
private let chunked_infix_parent_aligned
  (minor: minor_state) (addr: U64.t)
  : Lemma
      (requires is_infix_in_minor minor addr)
      (ensures
        U64.v addr % U64.v mword == 0 /\
        U64.v (infix_parent minor addr) % U64.v mword == 0)
  =
  assert (U64.v mword == 8);
...
let chunked_fwd_targets_above_minor_expected_stable
  ...
  if is_minor_pointer old_val then begin
    ...
    if new_val <> 0UL then begin
      ...
      chunked_fwd_targets_above_minor_elim fwd old_val;
      to_minor_offset_stable_above_minor new_val;
      assert (to_minor_offset new_val == new_val);
      assert (~(is_minor_pointer new_val));
      assert (to_minor_offset expected == new_val);
      assert (~(is_minor_pointer (to_minor_offset new_val) /\
               fwd (to_minor_offset new_val) <> 0UL));
      assert (~(is_minor_pointer (to_minor_offset expected) /\
               fwd (to_minor_offset expected) <> 0UL));
      ChunkedUpdate.chunked_update_value_stable_intro fwd expected
    end else begin
      assert (~(is_minor_pointer old_val /\ fwd old_val <> 0UL));
      assert (expected == old);
      assert (to_minor_offset expected == old_val);
      assert (~(is_minor_pointer (to_minor_offset expected) /\
               fwd (to_minor_offset expected) <> 0UL));
      ChunkedUpdate.chunked_update_value_stable_intro fwd expected
    end
...
[@"opaque_to_smt"]
let chunked_fwd_targets_valid_addr (fwd: forwarding_map) : prop =
  forall (x: U64.t). fwd x <> 0UL ==> is_val_addr (fwd x)

let chunked_fwd_targets_valid_addr_empty ()
  : Lemma (ensures chunked_fwd_targets_valid_addr empty_forwarding)
  =
  reveal_opaque (`%chunked_fwd_targets_valid_addr)
    (chunked_fwd_targets_valid_addr empty_forwarding);
  let aux (x: U64.t)
    : Lemma
        (requires empty_forwarding x <> 0UL)
        (ensures is_val_addr (empty_forwarding x))
    =
    assert False
  in
  FStar.Classical.forall_intro (FStar.Classical.move_requires aux)

let chunked_fwd_targets_valid_addr_elim
  (fwd: forwarding_map) (x: U64.t)
  : Lemma
      (requires chunked_fwd_targets_valid_addr fwd /\
                fwd x <> 0UL)
      (ensures is_val_addr (fwd x))
  =
  reveal_opaque (`%chunked_fwd_targets_valid_addr)
    (chunked_fwd_targets_valid_addr fwd)

private let chunked_fwd_targets_valid_addr_extend
  (fwd: forwarding_map) (addr target: U64.t)
  : Lemma
      (requires
        chunked_fwd_targets_valid_addr fwd /\
        is_val_addr target)
      (ensures
        chunked_fwd_targets_valid_addr
  ...
  let aux (x: U64.t)
    : Lemma
        (requires extend_forwarding fwd addr target x <> 0UL)
        (ensures is_val_addr (extend_forwarding fwd addr target x))
    =
    if x = addr then
      assert (extend_forwarding fwd addr target x == target)
    else begin
      assert (extend_forwarding fwd addr target x == fwd x);
      chunked_fwd_targets_valid_addr_elim fwd x
    end
  in
  FStar.Classical.forall_intro (FStar.Classical.move_requires aux)

private let chunked_fwd_targets_above_minor_extend
  (fwd: forwarding_map) (addr target: U64.t)
  : Lemma
      (requires
        chunked_fwd_targets_above_minor fwd /\
        U64.v target >= minor_heap_size /\
        U64.v target % U64.v mword == 0)
      (ensures
  ...
  let aux (x: U64.t)
    : Lemma
        (requires extend_forwarding fwd addr target x <> 0UL)
        (ensures
         U64.v (extend_forwarding fwd addr target x) >= minor_heap_size /\
         U64.v (extend_forwarding fwd addr target x) % U64.v mword == 0)
    =
    if x = addr then
      assert (extend_forwarding fwd addr target x == target)
    else begin
      assert (extend_forwarding fwd addr target x == fwd x);
      chunked_fwd_targets_above_minor_elim fwd x
    end
  in
  FStar.Classical.forall_intro (FStar.Classical.move_requires aux)

private let chunked_fwd_infix_target_above_minor
  (base addr parent: U64.t)
  : Lemma
      (requires
        U64.v base >= minor_heap_size /\
        U64.v base % U64.v mword == 0 /\
        U64.v parent % U64.v mword == 0 /\
        U64.v addr >= U64.v parent /\
        U64.v addr % U64.v mword == 0 /\
        U64.v base + (U64.v addr - U64.v parent) < heap_size)
      (ensures
        (let target =
         U64.uint_to_t (U64.v base + (U64.v addr - U64.v parent)) in
        U64.v target >= minor_heap_size /\
        U64.v target % U64.v mword == 0))
  =
  let delta = U64.v addr - U64.v parent in
  assert (U64.v mword == 8);
  FStar.Math.Lemmas.modulo_addition_lemma (U64.v parent) 8 delta;
  assert ((U64.v parent + delta) % 8 == U64.v addr % 8);
  assert (delta % 8 == 0);
  FStar.Math.Lemmas.modulo_addition_lemma (U64.v base) 8 delta;
  assert ((U64.v base + delta) % 8 == 0);
  assert (U64.v base + delta >= minor_heap_size);
  assert (U64.v (U64.uint_to_t (U64.v base + delta)) ==
         U64.v base + delta)

private let chunked_infix_target_valid_addr
  (base addr parent: U64.t)
  : Lemma
      (requires
        is_val_addr base /\
  ...
  FStar.Math.Lemmas.modulo_addition_lemma (U64.v base) 8 delta;
  assert ((U64.v base + delta) % 8 == 0);
  assert (U64.v base + delta >= U64.v mword);
  assert (U64.v base + delta < heap_size);
  assert (U64.v (U64.uint_to_t (U64.v base + delta)) ==
          U64.v base + delta);
  is_val_addr_spec (U64.uint_to_t (U64.v base + delta))

[@"opaque_to_smt"]
let chunked_fwd_noninfix_targets_in_major
...
let chunked_fwd_noninfix_targets_in_major_empty
  (minor: minor_state) (mh: MH.major_heap)
  : Lemma
      (ensures
        chunked_fwd_noninfix_targets_in_major minor empty_forwarding mh)
  =
  reveal_opaque (`%chunked_fwd_noninfix_targets_in_major)
    (chunked_fwd_noninfix_targets_in_major minor empty_forwarding mh);
  let aux (x: U64.t)
    : Lemma
        (requires empty_forwarding x <> 0UL /\
        ...
        (ensures
          U64.v (empty_forwarding x) >= U64.v mword /\
          U64.v (empty_forwarding x) < heap_size /\
          U64.v (empty_forwarding x) % U64.v mword == 0 /\
          Seq.mem ((empty_forwarding x) <: obj_addr)
            (MH.major_objects mh))
    =
    assert (empty_forwarding x == 0UL);
    assert False
...
let chunked_fwd_noninfix_targets_in_major_elim
  (minor: minor_state) (fwd: forwarding_map) (mh: MH.major_heap)
  (x: U64.t)
  : Lemma
      (requires
...
private let chunked_cheney_forward_normal_preserves_fwd_targets_above_minor
  (minor: minor_state) (cs: ChunkedCheney.chunked_cheney_state)
  ...
  if not (Seq.mem addr (minor_objects minor)) || cs.ccs_fwd addr <> 0UL then begin
    ...
    assert ((ChunkedCheney.chunked_cheney_forward_normal
             minor cs addr fuel).ccs_fwd == cs.ccs_fwd)
  end else begin
    ...
    if wz = 0 then begin
      ...
      chunked_fwd_target_above_minor_from_major_addr cs.ccs_fp;
      let res =
        ChunkedPromote.chunked_promote_object_with_fuel
         minor cs.ccs_major addr cs.ccs_fp wz fuel in
      assert (res.new_addr == cs.ccs_fp);
      ...
      chunked_fwd_targets_above_minor_extend
...
#push-options "--z3rlimit 10 --fuel 1 --ifuel 0 --split_queries always"
let chunked_cheney_forward_one_preserves_fwd_targets_above_minor
  (minor: minor_state) (cs: ChunkedCheney.chunked_cheney_state)
  (addr: U64.t) (fuel remaining: nat)
  : Lemma
      (requires
  ...
  if cs.ccs_fwd addr <> 0UL then begin
    ...
    let parent = infix_parent minor addr in
    chunked_infix_parent_aligned minor addr;
    assert (U64.v parent % 8 == 0);
    assert (U64.v addr % 8 == 0);
    chunked_cheney_forward_normal_preserves_fwd_targets_above_minor
      minor cs parent fuel remaining;
    chunked_cheney_forward_normal_head_split_preserves_remaining_head_wosize
      minor cs parent fuel remaining;
    let cs' =
      ChunkedCheney.chunked_cheney_forward_normal minor cs parent fuel in
    assert (chunked_fwd_targets_above_minor cs'.ccs_fwd);
    if cs'.ccs_fwd parent <> 0UL &&
       U64.v addr >= U64.v parent &&
       U64.v (cs'.ccs_fwd parent) + (U64.v addr - U64.v parent) < heap_size
    then begin
      ChunkedCheney.chunked_cheney_forward_one_infix_guard_pass
        minor cs addr fuel;
      chunked_fwd_targets_above_minor_elim cs'.ccs_fwd parent;
      let sum =
        U64.uint_to_t
         (U64.v (cs'.ccs_fwd parent) +
          (U64.v addr - U64.v parent)) in
      chunked_fwd_infix_target_above_minor
        (cs'.ccs_fwd parent) addr parent;
      assert ((ChunkedCheney.chunked_cheney_forward_one
               minor cs addr fuel).ccs_fwd ==
             extend_forwarding cs'.ccs_fwd addr sum);
      chunked_fwd_targets_above_minor_extend cs'.ccs_fwd addr sum
    end else begin
      ChunkedCheney.chunked_cheney_forward_one_infix_guard_fail
        minor cs addr fuel;
      assert ((ChunkedCheney.chunked_cheney_forward_one
...
#push-options "--z3rlimit 10 --fuel 1 --ifuel 0 --split_queries always"
private let chunked_cheney_forward_normal_preserves_fwd_targets_valid_addr
  (minor: minor_state) (cs: ChunkedCheney.chunked_cheney_state)
  (addr: U64.t) (fuel remaining: nat)
  : Lemma
      (requires
        ...
        SpecMajorAlloc.major_fl_chain_terminates
          cs.ccs_major cs.ccs_fp fuel = true /\
        chunked_fwd_targets_valid_addr cs.ccs_fwd /\
        remaining > 0 /\
        SpecMajorAlloc.major_fl_head_wosize
          cs.ccs_major cs.ccs_fp >= remaining /\
        (Seq.mem addr (minor_objects minor) /\
  ...
  if not (Seq.mem addr (minor_objects minor)) || cs.ccs_fwd addr <> 0UL then begin
    ...
    if wz = 0 then begin
      ...
      SpecMajorAlloc.major_fl_above_zero_current
        cs.ccs_major cs.ccs_fp fuel;
      is_val_addr_spec cs.ccs_fp;
      assert (is_val_addr cs.ccs_fp);
      let res =
        ChunkedPromote.chunked_promote_object_with_fuel
          minor cs.ccs_major addr cs.ccs_fp wz fuel in
      assert (res.new_addr == cs.ccs_fp);
      ...
      assert ((ChunkedCheney.chunked_cheney_forward_normal
               minor cs addr fuel).ccs_fwd ==
              extend_forwarding cs.ccs_fwd addr cs.ccs_fp);
      chunked_fwd_targets_valid_addr_extend
        cs.ccs_fwd addr cs.ccs_fp
    end
  end

let chunked_cheney_forward_one_preserves_fwd_targets_valid_addr
  (minor: minor_state) (cs: ChunkedCheney.chunked_cheney_state)
  (addr: U64.t) (fuel remaining: nat)
  : Lemma
      (requires
  ...
  if cs.ccs_fwd addr <> 0UL then begin
    ...
    assert (U64.v parent % 8 == 0);
    assert (U64.v addr % 8 == 0);
    chunked_cheney_forward_normal_preserves_fwd_targets_valid_addr
      minor cs parent fuel remaining;
    chunked_cheney_forward_normal_head_split_preserves_remaining_head_wosize
      minor cs parent fuel remaining;
    let cs' =
      ChunkedCheney.chunked_cheney_forward_normal minor cs parent fuel in
    assert (chunked_fwd_targets_valid_addr cs'.ccs_fwd);
    if cs'.ccs_fwd parent <> 0UL &&
       U64.v addr >= U64.v parent &&
       U64.v (cs'.ccs_fwd parent) + (U64.v addr - U64.v parent) < heap_size
    then begin
      ChunkedCheney.chunked_cheney_forward_one_infix_guard_pass
        minor cs addr fuel;
      chunked_fwd_targets_valid_addr_elim cs'.ccs_fwd parent;
      let sum =
        U64.uint_to_t
          (U64.v (cs'.ccs_fwd parent) +
           (U64.v addr - U64.v parent)) in
      chunked_infix_target_valid_addr
        (cs'.ccs_fwd parent) addr parent;
      assert ((ChunkedCheney.chunked_cheney_forward_one
               minor cs addr fuel).ccs_fwd ==
              extend_forwarding cs'.ccs_fwd addr sum);
      chunked_fwd_targets_valid_addr_extend cs'.ccs_fwd addr sum
    end else begin
      ChunkedCheney.chunked_cheney_forward_one_infix_guard_fail
        minor cs addr fuel;
      assert ((ChunkedCheney.chunked_cheney_forward_one
...
#push-options "--z3rlimit 10 --fuel 1 --ifuel 0 --split_queries always"
private let chunked_cheney_forward_normal_preserves_fwd_noninfix_targets_in_major
  (minor: minor_state) (cs: ChunkedCheney.chunked_cheney_state)
  (addr: U64.t) (fuel remaining: nat)
  : Lemma
      (requires
    ...
    if wz = 0 then begin
      ...
      let res =
        ChunkedPromote.chunked_promote_object_with_fuel
          minor cs.ccs_major addr cs.ccs_fp wz fuel in
      assert (res.new_addr == cs.ccs_fp);
      assert (Seq.mem (cs.ccs_fp <: obj_addr)
        (MH.major_objects res.major_out));
      ChunkedCheney.chunked_cheney_forward_normal_success
        minor cs addr fuel;
      assert (cs'.ccs_fwd ==
      ...
      let aux (x: U64.t)
        : Lemma
            (requires cs'.ccs_fwd x <> 0UL /\
                      ~(is_infix_in_minor minor x))
            (ensures
              U64.v (cs'.ccs_fwd x) >= U64.v mword /\
              U64.v (cs'.ccs_fwd x) < heap_size /\
              U64.v (cs'.ccs_fwd x) % U64.v mword == 0 /\
              Seq.mem ((cs'.ccs_fwd x) <: obj_addr)
                (MH.major_objects cs'.ccs_major))
        =
        if x = addr then begin
          assert (cs'.ccs_fwd x == cs.ccs_fp);
...
let chunked_cheney_forward_one_preserves_fwd_noninfix_targets_in_major
  (minor: minor_state) (cs: ChunkedCheney.chunked_cheney_state)
  (addr: U64.t) (fuel remaining: nat)
  : Lemma
      (requires
  ...
  if cs.ccs_fwd addr <> 0UL then begin
    ChunkedCheney.chunked_cheney_forward_one_noop minor cs addr fuel;
    assert (ChunkedCheney.chunked_cheney_forward_one
              minor cs addr fuel == cs)
  end else if is_infix_in_minor minor addr then begin
    let parent = infix_parent minor addr in
    chunked_cheney_forward_normal_preserves_fwd_noninfix_targets_in_major
      minor cs parent fuel remaining;
    chunked_cheney_forward_normal_head_split_preserves_remaining_head_wosize
      minor cs parent fuel remaining;
    let csn =
      ChunkedCheney.chunked_cheney_forward_normal minor cs parent fuel in
    assert (chunked_fwd_noninfix_targets_in_major
    ...
    then begin
      ...
      let aux (x: U64.t)
        : Lemma
            (requires r.ccs_fwd x <> 0UL /\
                      ~(is_infix_in_minor minor x))
            (ensures
              U64.v (r.ccs_fwd x) >= U64.v mword /\
              U64.v (r.ccs_fwd x) < heap_size /\
              U64.v (r.ccs_fwd x) % U64.v mword == 0 /\
              Seq.mem ((r.ccs_fwd x) <: obj_addr)
                (MH.major_objects r.ccs_major))
        =
        if x = addr then begin
          ...
          chunked_fwd_noninfix_targets_in_major_elim
            minor csn.ccs_fwd csn.ccs_major x
        end
      in
      FStar.Classical.forall_intro (FStar.Classical.move_requires aux)
    end else begin
...
let rec chunked_cheney_forward_roots_head_split_preserves_chain_objects_blue
  (minor: minor_state) (cs: ChunkedCheney.chunked_cheney_state)
  (roots: seq U64.t) (idx: nat) (alloc_fuel: nat)
  : Lemma
      (requires
...
let rec chunked_cheney_forward_fields_head_split_preserves_chain_objects_blue
  (minor: minor_state) (cs: ChunkedCheney.chunked_cheney_state)
  (parent: U64.t) (idx: nat) (wosize: nat) (alloc_fuel: nat)
  : Lemma
      (requires
...
let rec chunked_cheney_scan_head_split_preserves_chain_objects_blue
  (minor: minor_state) (cs: ChunkedCheney.chunked_cheney_state)
  (scan: nat) (scan_fuel: nat) (alloc_fuel: nat)
  : Lemma
      (requires
  ...
  if scan_fuel > 0 then
    if scan >= Seq.length cs.ccs_queue then
      ...
      assert (GenInv.chunked_chain_objects_blue
                cs'.ccs_major cs'.ccs_fp alloc_fuel);
      chunked_cheney_scan_head_split_preserves_chain_objects_blue
        minor cs' (scan + 1) fuel' alloc_fuel
    end
  else begin
    assert (scan_fuel = 0);
...
let chunked_cheney_promote_head_split_preserves_chain_objects_blue
  (minor: minor_state) (major: MH.major_heap) (fp: U64.t)
  (roots: seq U64.t) (alloc_fuel: nat)
  : Lemma
      (requires
...
private let chunked_cheney_forward_normal_head_split_preserves_old_major_objects
  (minor: minor_state) (cs: ChunkedCheney.chunked_cheney_state)
  (addr: U64.t) (fuel: nat)
  : Lemma
      (requires
...
private let chunked_cheney_forward_one_head_split_preserves_old_major_objects
  (minor: minor_state) (cs: ChunkedCheney.chunked_cheney_state)
  (addr: U64.t) (fuel: nat)
  : Lemma
      (requires
  ...
  if cs.ccs_fwd addr <> 0UL then
    ...
    let parent = infix_parent minor addr in
    chunked_cheney_forward_normal_head_split_preserves_old_major_objects
      minor cs parent fuel;
    let cs' = ChunkedCheney.chunked_cheney_forward_normal minor cs parent fuel in
    if cs'.ccs_fwd parent <> 0UL &&
       U64.v addr >= U64.v parent &&
...
private let rec chunked_cheney_forward_roots_head_split_preserves_old_major_objects
  (minor: minor_state) (cs: ChunkedCheney.chunked_cheney_state)
  (roots: seq U64.t) (idx: nat) (alloc_fuel: nat)
  : Lemma
      (requires
  ...
  if idx >= Seq.length roots then
    ...
    chunked_cheney_forward_roots_head_split_preserves_old_major_objects
      minor cs' roots (idx + 1) alloc_fuel;
    let old_to_step = ChunkedCheney.chunked_cheney_forward_one minor cs r alloc_fuel in
    let final =
      ChunkedCheney.chunked_cheney_forward_roots
        minor old_to_step roots (idx + 1) alloc_fuel in
    let aux (src: obj_addr)
      : Lemma
          (requires Seq.mem src (MH.major_objects cs.ccs_major))
          (ensures Seq.mem src (MH.major_objects final.ccs_major))
      =
      assert (Seq.mem src (MH.major_objects old_to_step.ccs_major));
      assert (Seq.mem src (MH.major_objects final.ccs_major))
    in
    FStar.Classical.forall_intro (FStar.Classical.move_requires aux)
  end

private let rec chunked_cheney_forward_fields_head_split_preserves_old_major_objects
  (minor: minor_state) (cs: ChunkedCheney.chunked_cheney_state)
  (parent: U64.t) (idx: nat) (wosize: nat) (alloc_fuel: nat)
  : Lemma
      (requires
  ...
  if idx >= wosize then
    ...
    let final =
      ChunkedCheney.chunked_cheney_forward_fields
        minor cs' parent (idx + 1) wosize alloc_fuel in
    let aux (src: obj_addr)
      : Lemma
          (requires Seq.mem src (MH.major_objects cs.ccs_major))
          (ensures Seq.mem src (MH.major_objects final.ccs_major))
...
private let rec chunked_cheney_scan_head_split_preserves_old_major_objects
  (minor: minor_state) (cs: ChunkedCheney.chunked_cheney_state)
  (scan: nat) (scan_fuel: nat) (alloc_fuel: nat)
  : Lemma
      (requires
  ...
  if scan_fuel > 0 then
    if scan >= Seq.length cs.ccs_queue then
      ...
      chunked_cheney_forward_fields_head_split_preserves_chunked_alloc_shape
        minor cs obj 0 wz alloc_fuel;
      chunked_cheney_scan_head_split_preserves_old_major_objects
        minor cs' (scan + 1) fuel' alloc_fuel;
      let final =
        ChunkedCheney.chunked_cheney_scan
...
let chunked_cheney_promote_head_split_preserves_old_major_objects
  (minor: minor_state) (major: MH.major_heap) (fp: U64.t)
  (roots: seq U64.t) (alloc_fuel: nat)
  : Lemma
      (requires
  ...
  let aux (src: obj_addr)
    : Lemma
        ...
        (ensures Seq.mem src (MH.major_objects res.major_final))
    =
...
private let chunked_cheney_forward_normal_head_split_preserves_old_non_blue_header
  (minor: minor_state) (cs: ChunkedCheney.chunked_cheney_state)
  ...
  if not (Seq.mem addr (minor_objects minor)) || cs.ccs_fwd addr <> 0UL then
    ...
    if wz = 0 then
      ...
      assert (SpecMajorAlloc.major_fl_head_wosize
                cs.ccs_major cs.ccs_fp >= wz + 2);
      chunked_promote_object_head_split_preserves_old_non_blue_header
...
private let chunked_cheney_forward_one_head_split_preserves_old_non_blue_header
  (minor: minor_state) (cs: ChunkedCheney.chunked_cheney_state)
  (addr: U64.t) (fuel: nat) (src: obj_addr) (hdr: U64.t)
  : Lemma
      (requires
  ...
  if cs.ccs_fwd addr <> 0UL then
    ...
    let parent = infix_parent minor addr in
    chunked_cheney_forward_normal_head_split_preserves_old_non_blue_header
...
private let rec chunked_cheney_forward_roots_head_split_preserves_old_non_blue_header
  (minor: minor_state) (cs: ChunkedCheney.chunked_cheney_state)
  (roots: seq U64.t) (idx: nat) (alloc_fuel: nat)
  (src: obj_addr) (hdr: U64.t)
  : Lemma
  ...
  if idx >= Seq.length roots then
    ...
    chunked_cheney_forward_roots_head_split_preserves_old_non_blue_header
      minor cs' roots (idx + 1) alloc_fuel src hdr
  end

private let rec chunked_cheney_forward_fields_head_split_preserves_old_non_blue_header
  (minor: minor_state) (cs: ChunkedCheney.chunked_cheney_state)
  (parent: U64.t) (idx: nat) (wosize: nat) (alloc_fuel: nat)
  (src: obj_addr) (hdr: U64.t)
  : Lemma
  ...
  if idx >= wosize then
    ...
    chunked_cheney_forward_fields_head_split_preserves_old_non_blue_header
      minor cs' parent (idx + 1) wosize alloc_fuel src hdr
  end

private let rec chunked_cheney_scan_head_split_preserves_old_non_blue_header
  (minor: minor_state) (cs: ChunkedCheney.chunked_cheney_state)
  (scan: nat) (scan_fuel: nat) (alloc_fuel: nat)
  (src: obj_addr) (hdr: U64.t)
  : Lemma
  ...
  if scan_fuel > 0 then
    if scan >= Seq.length cs.ccs_queue then
      ...
      assert (GenInv.chunked_chain_objects_blue
                cs'.ccs_major cs'.ccs_fp alloc_fuel);
      chunked_cheney_scan_head_split_preserves_old_non_blue_header
        minor cs' (scan + 1) fuel' alloc_fuel src hdr
    end
  else begin
    assert (scan_fuel = 0);
    ChunkedCheney.chunked_cheney_scan_base
...
let chunked_cheney_promote_head_split_preserves_old_non_blue_header
  (minor: minor_state) (major: MH.major_heap) (fp: U64.t)
  ...
  assert (chunked_cheney_scan_split_ready
            minor cs1 0 (cheney_fuel minor) alloc_fuel);
  chunked_cheney_scan_head_split_preserves_old_non_blue_header
    minor cs1 0 (cheney_fuel minor) alloc_fuel src hdr;
  let cs2 =
    ChunkedCheney.chunked_cheney_scan
      minor cs1 0 (cheney_fuel minor) alloc_fuel in
  let res =
  ...
  : Lemma
      (ensures
        (let r =
         ...
         forall (src: obj_addr).
           Seq.mem src (MH.major_objects major) ==>
           Seq.mem src (MH.major_objects r.capacity_major_out)))
  =
  let r =
    SpecMajorAlloc.ensure_major_head_capacity_spec
      major fp alloc_fuel needed fresh in
...
let rec chunked_cheney_forward_roots_preserves_fwd_targets_above_minor
  (minor: minor_state) (cs: ChunkedCheney.chunked_cheney_state)
  (roots: seq U64.t) (idx: nat) (alloc_fuel: nat) (remaining: nat)
  : Lemma
      (requires
  ...
  if idx >= Seq.length roots then
    ...
    assert (chunked_cheney_forward_roots_budget_ready
              minor cs' roots (idx + 1) alloc_fuel remaining);
    chunked_cheney_forward_one_preserves_fwd_targets_above_minor
      minor cs r alloc_fuel remaining;
    chunked_cheney_forward_one_head_split_preserves_remaining_head_wosize
      minor cs r alloc_fuel remaining;
    assert (chunked_fwd_targets_above_minor cs'.ccs_fwd);
    ...
    assert (SpecMajorAlloc.major_fl_chain_terminates
              cs'.ccs_major cs'.ccs_fp alloc_fuel = true);
    chunked_cheney_forward_roots_preserves_fwd_targets_above_minor
      minor cs' roots (idx + 1) alloc_fuel remaining
  end

let rec chunked_cheney_forward_fields_preserves_fwd_targets_above_minor
  (minor: minor_state) (cs: ChunkedCheney.chunked_cheney_state)
  (parent: U64.t) (idx: nat) (wosize: nat) (alloc_fuel: nat)
  (remaining: nat)
  : Lemma
  ...
  if idx >= wosize then
    ...
    assert (SpecMajorAlloc.major_fl_chain_terminates
              cs'.ccs_major cs'.ccs_fp alloc_fuel = true);
    chunked_cheney_forward_fields_preserves_fwd_targets_above_minor
      minor cs' parent (idx + 1) wosize alloc_fuel remaining
  end

let rec chunked_cheney_scan_preserves_fwd_targets_above_minor
  (minor: minor_state) (cs: ChunkedCheney.chunked_cheney_state)
  (scan: nat) (scan_fuel: nat) (alloc_fuel: nat) (remaining: nat)
  : Lemma
      (requires
  ...
  if scan_fuel > 0 then
    if scan >= Seq.length cs.ccs_queue then
      ...
      assert (SpecMajorAlloc.major_fl_chain_terminates
                cs'.ccs_major cs'.ccs_fp alloc_fuel = true);
      chunked_cheney_scan_preserves_fwd_targets_above_minor
        minor cs' (scan + 1) fuel' alloc_fuel remaining
    end
  else begin
    assert (scan_fuel = 0);
...
let chunked_cheney_promote_preserves_fwd_targets_above_minor
  (minor: minor_state) (major: MH.major_heap) (fp: U64.t)
  (roots: seq U64.t) (alloc_fuel: nat) (remaining: nat)
  : Lemma
      (requires
  ...
  let cs0 : ChunkedCheney.chunked_cheney_state =
    { ccs_major = major; ccs_fp = fp;
      ccs_fwd = empty_forwarding; ccs_queue = Seq.empty } in
  chunked_fwd_targets_above_minor_empty ();
  assert (chunked_fwd_targets_above_minor cs0.ccs_fwd);
  assert (chunked_cheney_forward_roots_budget_ready
            minor cs0 roots 0 alloc_fuel remaining);
  chunked_cheney_forward_roots_preserves_fwd_targets_above_minor
    minor cs0 roots 0 alloc_fuel remaining;
  ...
  let cs1 =
    ChunkedCheney.chunked_cheney_forward_roots
      minor cs0 roots 0 alloc_fuel in
  assert (chunked_fwd_targets_above_minor cs1.ccs_fwd);
  assert (GenInv.chunked_major_alloc_shape
            cs1.ccs_major cs1.ccs_fp alloc_fuel);
  assert (SpecMajorAlloc.major_fl_chain_terminates
            cs1.ccs_major cs1.ccs_fp alloc_fuel = true);
...
let rec chunked_cheney_forward_roots_preserves_fwd_targets_valid_addr
  (minor: minor_state) (cs: ChunkedCheney.chunked_cheney_state)
  (roots: seq U64.t) (idx: nat) (alloc_fuel: nat) (remaining: nat)
  : Lemma
      (requires
  ...
  if idx >= Seq.length roots then
    ...
    assert (chunked_cheney_forward_roots_budget_ready
              minor cs' roots (idx + 1) alloc_fuel remaining);
    chunked_cheney_forward_one_preserves_fwd_targets_valid_addr
      minor cs r alloc_fuel remaining;
    chunked_cheney_forward_one_head_split_preserves_remaining_head_wosize
      minor cs r alloc_fuel remaining;
    assert (chunked_fwd_targets_valid_addr cs'.ccs_fwd);
    ...
    assert (SpecMajorAlloc.major_fl_chain_terminates
              cs'.ccs_major cs'.ccs_fp alloc_fuel = true);
    chunked_cheney_forward_roots_preserves_fwd_targets_valid_addr
      minor cs' roots (idx + 1) alloc_fuel remaining
  end

let rec chunked_cheney_forward_fields_preserves_fwd_targets_valid_addr
  (minor: minor_state) (cs: ChunkedCheney.chunked_cheney_state)
  (parent: U64.t) (idx: nat) (wosize: nat) (alloc_fuel: nat)
  (remaining: nat)
  : Lemma
  ...
  if idx >= wosize then
    ...
    assert (SpecMajorAlloc.major_fl_chain_terminates
              cs'.ccs_major cs'.ccs_fp alloc_fuel = true);
    chunked_cheney_forward_fields_preserves_fwd_targets_valid_addr
      minor cs' parent (idx + 1) wosize alloc_fuel remaining
  end

let rec chunked_cheney_scan_preserves_fwd_targets_valid_addr
  (minor: minor_state) (cs: ChunkedCheney.chunked_cheney_state)
  (scan: nat) (scan_fuel: nat) (alloc_fuel: nat) (remaining: nat)
  : Lemma
      (requires
  ...
  if scan_fuel > 0 then
    if scan >= Seq.length cs.ccs_queue then
      ...
      assert (SpecMajorAlloc.major_fl_chain_terminates
                cs'.ccs_major cs'.ccs_fp alloc_fuel = true);
      chunked_cheney_scan_preserves_fwd_targets_valid_addr
        minor cs' (scan + 1) fuel' alloc_fuel remaining
    end
  else begin
    assert (scan_fuel = 0);
...
let chunked_cheney_promote_preserves_fwd_targets_valid_addr
  (minor: minor_state) (major: MH.major_heap) (fp: U64.t)
  (roots: seq U64.t) (alloc_fuel: nat) (remaining: nat)
  : Lemma
      (requires
  ...
  let cs0 : ChunkedCheney.chunked_cheney_state =
    { ccs_major = major; ccs_fp = fp;
      ccs_fwd = empty_forwarding; ccs_queue = Seq.empty } in
  chunked_fwd_targets_valid_addr_empty ();
  assert (chunked_fwd_targets_valid_addr cs0.ccs_fwd);
  assert (chunked_cheney_forward_roots_budget_ready
            minor cs0 roots 0 alloc_fuel remaining);
  chunked_cheney_forward_roots_preserves_fwd_targets_valid_addr
    minor cs0 roots 0 alloc_fuel remaining;
  ...
  let cs1 =
    ChunkedCheney.chunked_cheney_forward_roots
      minor cs0 roots 0 alloc_fuel in
  assert (chunked_fwd_targets_valid_addr cs1.ccs_fwd);
  assert (GenInv.chunked_major_alloc_shape
            cs1.ccs_major cs1.ccs_fp alloc_fuel);
  assert (SpecMajorAlloc.major_fl_chain_terminates
            cs1.ccs_major cs1.ccs_fp alloc_fuel = true);
...
private let rec chunked_cheney_forward_roots_preserves_fwd_noninfix_targets_in_major
  (minor: minor_state) (cs: ChunkedCheney.chunked_cheney_state)
  (roots: seq U64.t) (idx: nat) (alloc_fuel: nat) (remaining: nat)
  : Lemma
      (requires
  ...
  if idx >= Seq.length roots then
    ...
    assert (chunked_cheney_forward_roots_budget_ready
              minor cs' roots (idx + 1) alloc_fuel remaining);
    chunked_cheney_forward_one_preserves_fwd_noninfix_targets_in_major
      minor cs r alloc_fuel remaining;
    chunked_cheney_forward_one_head_split_preserves_remaining_head_wosize
      minor cs r alloc_fuel remaining;
    assert (chunked_fwd_noninfix_targets_in_major
    ...
    assert (SpecMajorAlloc.major_fl_chain_terminates
              cs'.ccs_major cs'.ccs_fp alloc_fuel = true);
    chunked_cheney_forward_roots_preserves_fwd_noninfix_targets_in_major
      minor cs' roots (idx + 1) alloc_fuel remaining
  end

private let rec chunked_cheney_forward_fields_preserves_fwd_noninfix_targets_in_major
  (minor: minor_state) (cs: ChunkedCheney.chunked_cheney_state)
  (parent: U64.t) (idx: nat) (wosize: nat) (alloc_fuel: nat)
  (remaining: nat)
  : Lemma
  ...
  if idx >= wosize then
    ...
    assert (SpecMajorAlloc.major_fl_chain_terminates
              cs'.ccs_major cs'.ccs_fp alloc_fuel = true);
    chunked_cheney_forward_fields_preserves_fwd_noninfix_targets_in_major
      minor cs' parent (idx + 1) wosize alloc_fuel remaining
  end

private let rec chunked_cheney_scan_preserves_fwd_noninfix_targets_in_major
  (minor: minor_state) (cs: ChunkedCheney.chunked_cheney_state)
  (scan: nat) (scan_fuel: nat) (alloc_fuel: nat) (remaining: nat)
  : Lemma
      (requires
  ...
  if scan_fuel > 0 then
    if scan >= Seq.length cs.ccs_queue then
      ...
      assert (SpecMajorAlloc.major_fl_chain_terminates
                cs'.ccs_major cs'.ccs_fp alloc_fuel = true);
      chunked_cheney_scan_preserves_fwd_noninfix_targets_in_major
        minor cs' (scan + 1) fuel' alloc_fuel remaining
    end
  else begin
    assert (scan_fuel = 0);
...
let chunked_cheney_promote_preserves_fwd_noninfix_targets_in_major
  (minor: minor_state) (major: MH.major_heap) (fp: U64.t)
  (roots: seq U64.t) (alloc_fuel: nat) (remaining: nat)
  : Lemma
      (requires
  ...
  let cs0 : ChunkedCheney.chunked_cheney_state =
    { ccs_major = major; ccs_fp = fp;
      ccs_fwd = empty_forwarding; ccs_queue = Seq.empty } in
  chunked_fwd_noninfix_targets_in_major_empty minor major;
  assert (chunked_fwd_noninfix_targets_in_major
            minor cs0.ccs_fwd cs0.ccs_major);
  assert (chunked_cheney_forward_roots_budget_ready
            minor cs0 roots 0 alloc_fuel remaining);
  chunked_cheney_forward_roots_preserves_fwd_noninfix_targets_in_major
    minor cs0 roots 0 alloc_fuel remaining;
...
private let nat_lt_pow2_54_size64 (n: nat)
  : Lemma (requires n < pow2 54)
          (ensures FStar.UInt.size n 64)
  =
  FStar.Math.Lemmas.pow2_lt_compat 64 54;
  assert (n < pow2 64)

#push-options "--z3rlimit 10 --fuel 0 --ifuel 0 --split_queries always"
private let chunked_fwd_target_header_pre
  (minor: minor_state) (cs: ChunkedCheney.chunked_cheney_state)
  (x: U64.t)
  : prop =
  cs.ccs_fwd x <> 0UL /\
...
private let chunked_fwd_target_header_match
  (minor: minor_state) (cs: ChunkedCheney.chunked_cheney_state)
  (x: U64.t)
  : prop =
  chunked_fwd_target_header_pre minor cs x ==>
  U64.v (cs.ccs_fwd x) >= U64.v mword /\
  U64.v (cs.ccs_fwd x) < heap_size /\
  U64.v (cs.ccs_fwd x) % U64.v mword == 0 /\
  (let target : obj_addr = cs.ccs_fwd x in
...
private let chunked_fwd_target_headers_match_state
  (minor: minor_state) (cs: ChunkedCheney.chunked_cheney_state)
  : prop =
  forall (x: U64.t). chunked_fwd_target_header_match minor cs x

private let chunked_fwd_target_headers_match_state_elim
  (minor: minor_state) (cs: ChunkedCheney.chunked_cheney_state)
  (x: U64.t)
  : Lemma
      (requires
        chunked_fwd_target_headers_match_state minor cs /\
        chunked_fwd_target_header_pre minor cs x)
      (ensures
        U64.v (cs.ccs_fwd x) >= U64.v mword /\
        U64.v (cs.ccs_fwd x) < heap_size /\
        ...
        (let target : obj_addr = cs.ccs_fwd x in
         ...
         | None -> False)))
  =
  assert (chunked_fwd_target_header_match minor cs x)

private let chunked_fwd_target_headers_match_initial
  (minor: minor_state) (major: MH.major_heap) (fp: U64.t)
  : Lemma
      (ensures
        chunked_fwd_target_headers_match_state minor
          { ccs_major = major; ccs_fp = fp;
            ccs_fwd = empty_forwarding; ccs_queue = Seq.empty })
  =
  let cs0 : ChunkedCheney.chunked_cheney_state =
    { ccs_major = major; ccs_fp = fp;
      ccs_fwd = empty_forwarding; ccs_queue = Seq.empty } in
  let aux (x: U64.t)
    : Lemma
        (requires chunked_fwd_target_header_pre minor cs0 x)
        (ensures
          U64.v (cs0.ccs_fwd x) >= U64.v mword /\
          U64.v (cs0.ccs_fwd x) < heap_size /\
          U64.v (cs0.ccs_fwd x) % U64.v mword == 0 /\
          (let target : obj_addr = cs0.ccs_fwd x in
           Seq.mem target (MH.major_objects cs0.ccs_major) /\
           (match MH.read_word_in_major cs0.ccs_major (hd_address target) with
           | Some hdr ->
             getColor hdr <> Blue /\
             U64.v (getWosize hdr) == minor_wosize minor x
           | None -> False)))
    =
    assert (cs0.ccs_fwd x == 0UL);
    assert False
  in
  FStar.Classical.forall_intro
...
private let chunked_cheney_forward_one_budget_ready_implies_split_ready_for_header_match
  (minor: minor_state) (cs: ChunkedCheney.chunked_cheney_state)
  (addr: U64.t) (remaining: nat)
  : Lemma
      (requires chunked_cheney_forward_one_budget_ready minor cs addr remaining)
...
private let rec chunked_cheney_forward_fields_budget_ready_implies_split_ready_for_header_match
  (minor: minor_state) (cs: ChunkedCheney.chunked_cheney_state)
  (parent: U64.t) (idx: nat) (wosize: nat) (alloc_fuel: nat)
  (remaining: nat)
  : Lemma
  ...
  if idx < wosize then begin
    ...
    assert (chunked_cheney_forward_fields_budget_ready
              minor cs' parent (idx + 1) wosize alloc_fuel remaining);
    chunked_cheney_forward_one_budget_ready_implies_split_ready_for_header_match
      minor cs field_val remaining;
    chunked_cheney_forward_fields_budget_ready_implies_split_ready_for_header_match
      minor cs' parent (idx + 1) wosize alloc_fuel remaining
  end

private let rec chunked_cheney_forward_roots_budget_ready_implies_split_ready_for_header_match
  (minor: minor_state) (cs: ChunkedCheney.chunked_cheney_state)
  (roots: seq U64.t) (idx: nat) (alloc_fuel: nat) (remaining: nat)
  : Lemma
      (requires
  ...
  if idx < Seq.length roots then begin
    ...
    chunked_cheney_forward_one_budget_ready_implies_split_ready_for_header_match
      minor cs r remaining;
    chunked_cheney_forward_roots_budget_ready_implies_split_ready_for_header_match
      minor cs' roots (idx + 1) alloc_fuel remaining
  end

private let chunked_cheney_forward_normal_fresh_fwd_target_header_match
  (minor: minor_state) (cs: ChunkedCheney.chunked_cheney_state)
  (addr: U64.t) (fuel: nat)
  : Lemma
      (requires
      ...
      (ensures
        (let cs' =
           ChunkedCheney.chunked_cheney_forward_normal minor cs addr fuel in
         U64.v (cs'.ccs_fwd addr) >= U64.v mword /\
         U64.v (cs'.ccs_fwd addr) < heap_size /\
         U64.v (cs'.ccs_fwd addr) % U64.v mword == 0 /\
         (let target : obj_addr = cs'.ccs_fwd addr in
          Seq.mem target (MH.major_objects cs'.ccs_major) /\
          (match MH.read_word_in_major cs'.ccs_major (hd_address target) with
          | Some hdr ->
            getColor hdr <> Blue /\
            U64.v (getWosize hdr) == minor_wosize minor addr
          | None -> False))))
  =
  let cs' =
    ChunkedCheney.chunked_cheney_forward_normal minor cs addr fuel in
  ...
  is_val_addr_spec (cs'.ccs_fwd addr);
  let target : obj_addr = cs'.ccs_fwd addr in
  let target0 : obj_addr = cs.ccs_fp in
  assert (target == target0);
  assert (hd_address target == hd_address target0);
  assert (Seq.mem target (MH.major_objects cs'.ccs_major));
  match MH.read_word_in_major cs'.ccs_major (hd_address target0) with
  | Some final_hdr ->
    assert (U64.v (getWosize final_hdr) == minor_wosize minor addr);
...
private let chunked_cheney_forward_normal_old_fwd_target_header_match
  (minor: minor_state) (cs: ChunkedCheney.chunked_cheney_state)
  (addr: U64.t) (fuel: nat) (x: U64.t)
  : Lemma
      (requires
        fuel > 1 /\
        chunked_fwd_target_headers_match_state minor cs /\
  ...
  ChunkedCheney.chunked_cheney_forward_normal_other_fwd
    minor cs addr x fuel;
  assert (cs'.ccs_fwd x == cs.ccs_fwd x);
  assert (chunked_fwd_target_header_pre minor cs x);
  chunked_fwd_target_headers_match_state_elim minor cs x;
  let old_target : obj_addr = cs.ccs_fwd x in
  match MH.read_word_in_major cs.ccs_major (hd_address old_target) with
  | Some hdr ->
    assert (getColor hdr <> Blue);
    assert (U64.v (getWosize hdr) == minor_wosize minor x);
    assert (U64.v (getWosize hdr) >= 1);
    chunked_cheney_forward_normal_head_split_preserves_old_major_objects
      minor cs addr fuel;
    chunked_cheney_forward_normal_head_split_preserves_old_non_blue_header
      minor cs addr fuel old_target hdr;
...
private let chunked_cheney_forward_normal_preserves_fwd_target_headers_match_state
  (minor: minor_state) (cs: ChunkedCheney.chunked_cheney_state)
  (addr: U64.t) (fuel remaining: nat)
  : Lemma
      (requires
  ...
  let aux (x: U64.t)
    : Lemma
        (requires chunked_fwd_target_header_pre minor cs' x)
        (ensures
          U64.v (cs'.ccs_fwd x) >= U64.v mword /\
          U64.v (cs'.ccs_fwd x) < heap_size /\
          U64.v (cs'.ccs_fwd x) % U64.v mword == 0 /\
      ...
      if wz = 0 then begin
        ...
        if x = addr then begin
          ...
          | Some old_hdr ->
            ...
            assert (wz < pow2 54);
          assert (minor_wosize minor addr < pow2 54);
          nat_lt_pow2_54_size64 wz;
          assert (FStar.UInt.size wz 64);
          assert (wz == minor_wosize minor addr);
          assert (FStar.UInt.size (minor_wosize minor addr) 64);
          chunked_cheney_forward_normal_fresh_fwd_target_header_match
            minor cs addr fuel;
          assert (U64.v (cs'.ccs_fwd x) >= U64.v mword);
          assert (U64.v (cs'.ccs_fwd x) < heap_size);
          assert (U64.v mword == 8);
          assert (U64.v mword <> 0);
          assert (U64.v (cs'.ccs_fwd x) % U64.v mword == 0)
        end else begin
          chunked_cheney_forward_normal_old_fwd_target_header_match
            minor cs addr fuel x
        end
      end
    end
  in
...
#push-options "--z3rlimit 20 --fuel 1 --ifuel 0 --split_queries always"
private let chunked_cheney_forward_one_preserves_fwd_target_headers_match_state
  (minor: minor_state) (cs: ChunkedCheney.chunked_cheney_state)
  (addr: U64.t) (fuel remaining: nat)
  : Lemma
      (requires
  ...
  if cs.ccs_fwd addr <> 0UL then begin
    ...
    let parent = infix_parent minor addr in
    chunked_cheney_forward_normal_preserves_fwd_target_headers_match_state
      minor cs parent fuel remaining;
    chunked_cheney_forward_normal_head_split_preserves_remaining_head_wosize
      minor cs parent fuel remaining;
    chunked_cheney_forward_normal_head_split_preserves_chain_objects_blue
    ...
    let csn =
      ChunkedCheney.chunked_cheney_forward_normal minor cs parent fuel in
    assert (chunked_fwd_target_headers_match_state minor csn);
    assert (GenInv.chunked_major_alloc_shape
              csn.ccs_major csn.ccs_fp fuel);
    assert (SpecMajorAlloc.major_fl_chain_terminates
              csn.ccs_major csn.ccs_fp fuel = true);
    assert (GenInv.chunked_chain_objects_blue
              csn.ccs_major csn.ccs_fp fuel);
    if csn.ccs_fwd parent <> 0UL &&
       U64.v addr >= U64.v parent &&
    ...
    then begin
      ChunkedCheney.chunked_cheney_forward_one_infix_guard_pass
        minor cs addr fuel;
      assert (r.ccs_fwd ==
              extend_forwarding csn.ccs_fwd addr
                (U64.uint_to_t
      ...
      let aux (x: U64.t)
        : Lemma
            (requires chunked_fwd_target_header_pre minor r x)
            (ensures
              U64.v (r.ccs_fwd x) >= U64.v mword /\
              U64.v (r.ccs_fwd x) < heap_size /\
              U64.v (r.ccs_fwd x) % U64.v mword == 0 /\
              (let target : obj_addr = r.ccs_fwd x in
               Seq.mem target (MH.major_objects r.ccs_major) /\
               (match MH.read_word_in_major r.ccs_major (hd_address target) with
                | Some hdr ->
                  getColor hdr <> Blue /\
                  U64.v (getWosize hdr) == minor_wosize minor x
                | None -> False)))
        ...
        if x = addr then begin
          ...
          assert (r.ccs_fwd x == csn.ccs_fwd x);
          assert (chunked_fwd_target_header_pre minor csn x);
          chunked_fwd_target_headers_match_state_elim minor csn x
        end
      in
      FStar.Classical.forall_intro
        (FStar.Classical.move_requires aux)
...
#push-options "--z3rlimit 20 --fuel 1 --ifuel 0 --split_queries always"
private let rec chunked_cheney_forward_roots_preserves_fwd_target_headers_match_state
  (minor: minor_state) (cs: ChunkedCheney.chunked_cheney_state)
  (roots: seq U64.t) (idx: nat) (alloc_fuel: nat) (remaining: nat)
  : Lemma
      (requires
  ...
  if idx >= Seq.length roots then
    ...
    assert (chunked_cheney_forward_roots_budget_ready
              minor cs' roots (idx + 1) alloc_fuel remaining);
    chunked_cheney_forward_one_preserves_fwd_target_headers_match_state
      minor cs r alloc_fuel remaining;
    chunked_cheney_forward_one_head_split_preserves_remaining_head_wosize
      minor cs r alloc_fuel remaining;
    chunked_cheney_forward_one_head_split_preserves_chain_objects_blue
      minor cs r alloc_fuel;
    assert (chunked_fwd_target_headers_match_state minor cs');
    assert (GenInv.chunked_major_alloc_shape
              cs'.ccs_major cs'.ccs_fp alloc_fuel);
    assert (SpecMajorAlloc.major_fl_chain_terminates
              cs'.ccs_major cs'.ccs_fp alloc_fuel = true);
    assert (GenInv.chunked_chain_objects_blue
              cs'.ccs_major cs'.ccs_fp alloc_fuel);
    chunked_cheney_forward_roots_preserves_fwd_target_headers_match_state
      minor cs' roots (idx + 1) alloc_fuel remaining
  end

private let rec chunked_cheney_forward_fields_preserves_fwd_target_headers_match_state
  (minor: minor_state) (cs: ChunkedCheney.chunked_cheney_state)
  (parent: U64.t) (idx: nat) (wosize: nat) (alloc_fuel: nat)
  (remaining: nat)
  : Lemma
  ...
  if idx >= wosize then
    ...
    assert (GenInv.chunked_chain_objects_blue
              cs'.ccs_major cs'.ccs_fp alloc_fuel);
    chunked_cheney_forward_fields_preserves_fwd_target_headers_match_state
      minor cs' parent (idx + 1) wosize alloc_fuel remaining
  end

private let rec chunked_cheney_scan_preserves_fwd_target_headers_match_state
  (minor: minor_state) (cs: ChunkedCheney.chunked_cheney_state)
  (scan: nat) (scan_fuel: nat) (alloc_fuel: nat) (remaining: nat)
  : Lemma
      (requires
  ...
  if scan_fuel > 0 then
    if scan >= Seq.length cs.ccs_queue then
      ...
      assert (GenInv.chunked_chain_objects_blue
                cs'.ccs_major cs'.ccs_fp alloc_fuel);
      chunked_cheney_scan_preserves_fwd_target_headers_match_state
        minor cs' (scan + 1) fuel' alloc_fuel remaining
    end
  else begin
    assert (scan_fuel = 0);
...
#restart-solver
#push-options "--z3rlimit 10 --fuel 1 --ifuel 0 --split_queries always"
let chunked_cheney_promote_fwd_target_header_matches_minor
  (minor: minor_state) (major: MH.major_heap) (fp: U64.t)
  (roots: seq U64.t) (alloc_fuel: nat) (remaining: nat)
  (x: U64.t)
  : Lemma
  ...
  let cs0 : ChunkedCheney.chunked_cheney_state =
    { ccs_major = major; ccs_fp = fp;
      ccs_fwd = empty_forwarding; ccs_queue = Seq.empty } in
  chunked_fwd_target_headers_match_initial minor major fp;
  assert (chunked_fwd_target_headers_match_state minor cs0);
  assert (chunked_cheney_forward_roots_budget_ready
            minor cs0 roots 0 alloc_fuel remaining);
  chunked_cheney_forward_roots_preserves_fwd_target_headers_match_state
    minor cs0 roots 0 alloc_fuel remaining;
  ...
  let cs1 =
    ChunkedCheney.chunked_cheney_forward_roots
      minor cs0 roots 0 alloc_fuel in
  assert (chunked_fwd_target_headers_match_state minor cs1);
  assert (GenInv.chunked_major_alloc_shape
            cs1.ccs_major cs1.ccs_fp alloc_fuel);
  assert (SpecMajorAlloc.major_fl_chain_terminates
            cs1.ccs_major cs1.ccs_fp alloc_fuel = true);
  ...
  assert (res.fwd_map == cs2.ccs_fwd);
  assert (res.major_final == cs2.ccs_major);
  assert (chunked_fwd_target_header_pre minor cs2 x);
  chunked_fwd_target_headers_match_state_elim minor cs2 x
#pop-options

#push-options "--z3rlimit 10 --fuel 0 --ifuel 0 --split_queries always"
private let chunked_fwd_target_field_pre
  (minor: minor_state) (cs: ChunkedCheney.chunked_cheney_state)
  ...
  U64.v field_addr == U64.v (cs.ccs_fwd x) + j * U64.v mword

private let chunked_fwd_target_field_match
  (minor: minor_state) (cs: ChunkedCheney.chunked_cheney_state)
  (x: U64.t) (j: nat) (field_addr: hp_addr)
  : prop =
  chunked_fwd_target_field_pre minor cs x j field_addr ==>
  U64.v (cs.ccs_fwd x) >= U64.v mword /\
  U64.v (cs.ccs_fwd x) < heap_size /\
  U64.v (cs.ccs_fwd x) % U64.v mword == 0 /\
  (let target : obj_addr = cs.ccs_fwd x in
...
private let chunked_fwd_target_fields_match_state
  (minor: minor_state) (cs: ChunkedCheney.chunked_cheney_state)
  : prop =
  forall (x: U64.t) (j: nat) (field_addr: hp_addr).
    chunked_fwd_target_field_match minor cs x j field_addr

private let chunked_fwd_target_fields_match_state_elim
  (minor: minor_state) (cs: ChunkedCheney.chunked_cheney_state)
  (x: U64.t) (j: nat) (field_addr: hp_addr)
  : Lemma
      (requires
        chunked_fwd_target_fields_match_state minor cs /\
        chunked_fwd_target_field_pre minor cs x j field_addr)
      (ensures
        U64.v (cs.ccs_fwd x) >= U64.v mword /\
        U64.v (cs.ccs_fwd x) < heap_size /\
        ...
        (let target : obj_addr = cs.ccs_fwd x in
         ...
         (match MH.read_word_in_major cs.ccs_major (hd_address target) with
          ...
          | None -> False)))
  =
  assert (chunked_fwd_target_field_match minor cs x j field_addr)

private let chunked_fwd_target_fields_match_initial
  (minor: minor_state) (major: MH.major_heap) (fp: U64.t)
  : Lemma
      (ensures
        chunked_fwd_target_fields_match_state minor
          { ccs_major = major; ccs_fp = fp;
            ccs_fwd = empty_forwarding; ccs_queue = Seq.empty })
  =
  let cs0 : ChunkedCheney.chunked_cheney_state =
  ...
  let aux (x: U64.t) (j: nat) (field_addr: hp_addr)
    : Lemma
        (requires chunked_fwd_target_field_pre minor cs0 x j field_addr)
        (ensures
          U64.v (cs0.ccs_fwd x) >= U64.v mword /\
          U64.v (cs0.ccs_fwd x) < heap_size /\
          U64.v (cs0.ccs_fwd x) % U64.v mword == 0 /\
          (let target : obj_addr = cs0.ccs_fwd x in
           ...
           (match MH.read_word_in_major cs0.ccs_major (hd_address target) with
            | Some hdr ->
              ...
              j < U64.v (getWosize hdr) /\
              CG.chunked_major_field_slot target j == Some field_addr /\
              MH.read_word_in_major cs0.ccs_major field_addr ==
                Some (minor_read_field minor x j)
            | None -> False)))
    =
    assert (cs0.ccs_fwd x == 0UL);
  ...
  FStar.Classical.forall_intro_3
    ...
    #(fun _ _ -> hp_addr)
    #(fun x j field_addr ->
      chunked_fwd_target_field_match minor cs0 x j field_addr)
    (FStar.Classical.move_requires_3
      #(U64.t) #(fun _ -> nat) #(fun _ _ -> hp_addr)
      #(fun x j field_addr ->
        chunked_fwd_target_field_pre minor cs0 x j field_addr)
      #(fun x j field_addr ->
        U64.v (cs0.ccs_fwd x) >= U64.v mword /\
        U64.v (cs0.ccs_fwd x) < heap_size /\
        U64.v (cs0.ccs_fwd x) % U64.v mword == 0 /\
...
#restart-solver
#push-options "--z3rlimit 20 --fuel 1 --ifuel 0 --split_queries always"
private let chunked_cheney_forward_normal_preserves_fwd_target_fields_match_state
  (minor: minor_state) (cs: ChunkedCheney.chunked_cheney_state)
  (addr: U64.t) (fuel remaining: nat)
  : Lemma
      (requires
  ...
  let aux (x: U64.t) (j: nat) (field_addr: hp_addr)
    : Lemma
        (requires chunked_fwd_target_field_pre minor cs' x j field_addr)
        (ensures
          U64.v (cs'.ccs_fwd x) >= U64.v mword /\
          U64.v (cs'.ccs_fwd x) < heap_size /\
          U64.v (cs'.ccs_fwd x) % U64.v mword == 0 /\
    ...
    if not (Seq.mem addr (minor_objects minor)) || cs.ccs_fwd addr <> 0UL then begin
      ChunkedCheney.chunked_cheney_forward_normal_noop minor cs addr fuel;
      assert (cs' == cs);
      chunked_fwd_target_fields_match_state_elim minor cs x j field_addr
    end else begin
      let wz = minor_wosize minor addr in
      if wz = 0 then begin
        ChunkedCheney.chunked_cheney_forward_normal_noop_wz0
        ...
        if x = addr then begin
          ...
          assert (minor_wosize minor addr < pow2 54);
          FStar.Math.Lemmas.pow2_lt_compat 64 54;
          assert (minor_wosize minor addr < pow2 64);
          assert (FStar.UInt.size (minor_wosize minor addr) 64);
          ...
          is_val_addr_spec (cs'.ccs_fwd x);
          let target : obj_addr = cs'.ccs_fwd x in
          assert (target == (cs.ccs_fp <: obj_addr));
          assert (U64.v field_addr ==
                  U64.v target + j * U64.v mword);
          assert (Seq.mem target (MH.major_objects cs'.ccs_major));
          match MH.read_word_in_major cs'.ccs_major (hd_address target) with
          | Some hdr ->
            assert (getColor hdr == White);
            assert (getColor hdr <> Blue);
            assert (U64.v (getWosize hdr) == wz);
            assert (U64.v (getTag hdr) == minor_tag minor x);
            assert (j < U64.v (getWosize hdr));
            assert_norm (U64.v mword == 8);
            let field_offset = U64.v target + j * 8 in
            assert (field_offset == U64.v field_addr);
            SpecMajorAlloc.aligned_lt_heap_has_word_room (U64.v field_addr);
            assert (U64.v field_addr + 8 <= heap_size);
            assert (field_offset + 8 <= heap_size);
            CG.chunked_major_field_slot_intro target j field_addr;
            assert (CG.chunked_major_field_slot target j == Some field_addr);
            assert (MH.read_word_in_major cs'.ccs_major field_addr ==
                    Some (minor_read_field minor x j))
          | None ->
            assert False
        end else begin
          ChunkedCheney.chunked_cheney_forward_normal_other_fwd
            minor cs addr x fuel;
          assert (cs'.ccs_fwd x == cs.ccs_fwd x);
          assert (chunked_fwd_target_field_pre minor cs x j field_addr);
          chunked_fwd_target_fields_match_state_elim
            minor cs x j field_addr;
          let target : obj_addr = cs.ccs_fwd x in
          match MH.read_word_in_major cs.ccs_major (hd_address target) with
          | Some hdr ->
            assert (getColor hdr <> Blue);
            ...
            chunked_cheney_forward_normal_head_split_preserves_old_major_objects
              minor cs addr fuel;
            chunked_cheney_forward_normal_head_split_preserves_old_non_blue_header
              minor cs addr fuel target hdr;
            chunked_cheney_forward_normal_head_split_preserves_old_non_blue_field
              minor cs addr fuel target hdr j field_addr
              (minor_read_field minor x j);
            assert (Seq.mem target (MH.major_objects cs'.ccs_major));
            assert (MH.read_word_in_major cs'.ccs_major
                      (hd_address target) == Some hdr);
            assert (MH.read_word_in_major cs'.ccs_major field_addr ==
                    Some (minor_read_field minor x j))
          | None ->
            assert False
...
#push-options "--z3rlimit 20 --fuel 1 --ifuel 0 --split_queries always"
private let chunked_cheney_forward_one_preserves_fwd_target_fields_match_state
  (minor: minor_state) (cs: ChunkedCheney.chunked_cheney_state)
  (addr: U64.t) (fuel remaining: nat)
  : Lemma
      (requires
  ...
  if cs.ccs_fwd addr <> 0UL then begin
    ...
    let parent = infix_parent minor addr in
    chunked_cheney_forward_normal_preserves_fwd_target_fields_match_state
      minor cs parent fuel remaining;
    chunked_cheney_forward_normal_head_split_preserves_remaining_head_wosize
      minor cs parent fuel remaining;
    let csn =
      ChunkedCheney.chunked_cheney_forward_normal minor cs parent fuel in
    assert (chunked_fwd_target_fields_match_state minor csn);
    if csn.ccs_fwd parent <> 0UL &&
       U64.v addr >= U64.v parent &&
       U64.v (csn.ccs_fwd parent) + (U64.v addr - U64.v parent) < heap_size
    then begin
      ...
      let aux (x: U64.t) (j: nat) (field_addr: hp_addr)
        : Lemma
            (requires chunked_fwd_target_field_pre minor r x j field_addr)
            (ensures
              U64.v (r.ccs_fwd x) >= U64.v mword /\
              U64.v (r.ccs_fwd x) < heap_size /\
              U64.v (r.ccs_fwd x) % U64.v mword == 0 /\
              (let target : obj_addr = r.ccs_fwd x in
               ...
               (match MH.read_word_in_major r.ccs_major (hd_address target) with
                | Some hdr ->
                  ...
                  j < U64.v (getWosize hdr) /\
                  CG.chunked_major_field_slot target j == Some field_addr /\
                  MH.read_word_in_major r.ccs_major field_addr ==
                    Some (minor_read_field minor x j)
                | None -> False)))
        =
        if x = addr then begin
          ...
          assert (r.ccs_fwd x == csn.ccs_fwd x);
          assert (chunked_fwd_target_field_pre minor csn x j field_addr);
          chunked_fwd_target_fields_match_state_elim
            minor csn x j field_addr
        end
      in
      FStar.Classical.forall_intro_3
        #(U64.t)
        ...
        #(fun _ _ -> hp_addr)
        #(fun x j field_addr ->
          chunked_fwd_target_field_match minor r x j field_addr)
        (FStar.Classical.move_requires_3
          #(U64.t) #(fun _ -> nat) #(fun _ _ -> hp_addr)
          #(fun x j field_addr ->
            chunked_fwd_target_field_pre minor r x j field_addr)
          #(fun x j field_addr ->
            U64.v (r.ccs_fwd x) >= U64.v mword /\
            U64.v (r.ccs_fwd x) < heap_size /\
            U64.v (r.ccs_fwd x) % U64.v mword == 0 /\
...
private let rec chunked_cheney_forward_roots_preserves_fwd_target_fields_match_state
  (minor: minor_state) (cs: ChunkedCheney.chunked_cheney_state)
  (roots: seq U64.t) (idx: nat) (alloc_fuel: nat) (remaining: nat)
  : Lemma
      (requires
  ...
  if idx >= Seq.length roots then
    ...
    assert (chunked_cheney_forward_roots_budget_ready
              minor cs' roots (idx + 1) alloc_fuel remaining);
    chunked_cheney_forward_one_preserves_fwd_target_fields_match_state
      minor cs r alloc_fuel remaining;
    chunked_cheney_forward_one_head_split_preserves_remaining_head_wosize
      minor cs r alloc_fuel remaining;
    chunked_cheney_forward_one_head_split_preserves_chain_objects_blue
      minor cs r alloc_fuel;
    assert (chunked_fwd_target_fields_match_state minor cs');
    assert (GenInv.chunked_major_alloc_shape
              cs'.ccs_major cs'.ccs_fp alloc_fuel);
    assert (SpecMajorAlloc.major_fl_chain_terminates
              cs'.ccs_major cs'.ccs_fp alloc_fuel = true);
    assert (GenInv.chunked_chain_objects_blue
              cs'.ccs_major cs'.ccs_fp alloc_fuel);
    chunked_cheney_forward_roots_preserves_fwd_target_fields_match_state
      minor cs' roots (idx + 1) alloc_fuel remaining
  end

private let chunked_cheney_forward_one_budget_ready_implies_split_ready_for_field_match
  (minor: minor_state) (cs: ChunkedCheney.chunked_cheney_state)
  (addr: U64.t) (remaining: nat)
  : Lemma
      (requires chunked_cheney_forward_one_budget_ready minor cs addr remaining)
...
private let rec chunked_cheney_forward_fields_budget_ready_implies_split_ready_for_field_match
  (minor: minor_state) (cs: ChunkedCheney.chunked_cheney_state)
  (parent: U64.t) (idx: nat) (wosize: nat) (alloc_fuel: nat)
  (remaining: nat)
  : Lemma
  ...
  if idx < wosize then begin
    ...
    assert (chunked_cheney_forward_fields_budget_ready
              minor cs' parent (idx + 1) wosize alloc_fuel remaining);
    chunked_cheney_forward_one_budget_ready_implies_split_ready_for_field_match
      minor cs field_val remaining;
    chunked_cheney_forward_fields_budget_ready_implies_split_ready_for_field_match
      minor cs' parent (idx + 1) wosize alloc_fuel remaining
  end

private let rec chunked_cheney_forward_fields_preserves_fwd_target_fields_match_state
  (minor: minor_state) (cs: ChunkedCheney.chunked_cheney_state)
  (parent: U64.t) (idx: nat) (wosize: nat) (alloc_fuel: nat)
  (remaining: nat)
  : Lemma
  ...
  if idx >= wosize then
    ...
    assert (GenInv.chunked_chain_objects_blue
              cs'.ccs_major cs'.ccs_fp alloc_fuel);
    chunked_cheney_forward_fields_preserves_fwd_target_fields_match_state
      minor cs' parent (idx + 1) wosize alloc_fuel remaining
  end

private let rec chunked_cheney_scan_preserves_fwd_target_fields_match_state
  (minor: minor_state) (cs: ChunkedCheney.chunked_cheney_state)
  (scan: nat) (scan_fuel: nat) (alloc_fuel: nat) (remaining: nat)
  : Lemma
      (requires
  ...
  if scan_fuel > 0 then
    if scan >= Seq.length cs.ccs_queue then
      ...
      assert (GenInv.chunked_chain_objects_blue
                cs'.ccs_major cs'.ccs_fp alloc_fuel);
      chunked_cheney_scan_preserves_fwd_target_fields_match_state
        minor cs' (scan + 1) fuel' alloc_fuel remaining
    end
  else begin
    assert (scan_fuel = 0);
...
private let rec chunked_cheney_forward_roots_budget_ready_implies_split_ready_for_field_match
  (minor: minor_state) (cs: ChunkedCheney.chunked_cheney_state)
  (roots: seq U64.t) (idx: nat) (alloc_fuel: nat) (remaining: nat)
  : Lemma
      (requires
  ...
  if idx < Seq.length roots then begin
    ...
    chunked_cheney_forward_one_budget_ready_implies_split_ready_for_field_match
      minor cs r remaining;
    chunked_cheney_forward_roots_budget_ready_implies_split_ready_for_field_match
      minor cs' roots (idx + 1) alloc_fuel remaining
  end

let chunked_cheney_promote_fwd_target_fields_match
  (minor: minor_state) (major: MH.major_heap) (fp: U64.t)
  (roots: seq U64.t) (alloc_fuel: nat) (remaining: nat)
  (x: U64.t) (j: nat) (field_addr: hp_addr)
  : Lemma
  ...
  let cs0 : ChunkedCheney.chunked_cheney_state =
    { ccs_major = major; ccs_fp = fp;
      ccs_fwd = empty_forwarding; ccs_queue = Seq.empty } in
  chunked_fwd_target_fields_match_initial minor major fp;
  assert (chunked_fwd_target_fields_match_state minor cs0);
  assert (chunked_cheney_forward_roots_budget_ready
            minor cs0 roots 0 alloc_fuel remaining);
  chunked_cheney_forward_roots_preserves_fwd_target_fields_match_state
    minor cs0 roots 0 alloc_fuel remaining;
  ...
  let cs1 =
    ChunkedCheney.chunked_cheney_forward_roots
      minor cs0 roots 0 alloc_fuel in
  assert (chunked_fwd_target_fields_match_state minor cs1);
  assert (GenInv.chunked_major_alloc_shape
            cs1.ccs_major cs1.ccs_fp alloc_fuel);
  assert (SpecMajorAlloc.major_fl_chain_terminates
            cs1.ccs_major cs1.ccs_fp alloc_fuel = true);
...
let rec chunked_cheney_scan_end_exhausted_or_fuel
  ...
  if scan_fuel > 0 then
    if scan >= Seq.length cs.ccs_queue then begin
      ...
      FStar.Math.Lemmas.paren_add_right scan 1 fuel';
...
let chunked_cheney_promote_after_minor_promotion_head_preflight
  ...
  assert (chunked_cheney_promote_budget_ready
            minor r.capacity_major_out r.capacity_fp_out roots
            r.capacity_fuel_out 1);
  chunked_cheney_promote_budget_ready_implies_split_ready
    minor r.capacity_major_out r.capacity_fp_out roots
    r.capacity_fuel_out 1;
  chunked_cheney_promote_head_split_preserves_remaining_head_wosize
    minor r.capacity_major_out r.capacity_fp_out roots
    r.capacity_fuel_out 1;
  chunked_cheney_promote_preserves_fwd_targets_above_minor
    minor r.capacity_major_out r.capacity_fp_out roots
    r.capacity_fuel_out 1;
  chunked_cheney_promote_preserves_fwd_targets_valid_addr
    minor r.capacity_major_out r.capacity_fp_out roots
    r.capacity_fuel_out 1;
  chunked_cheney_promote_preserves_fwd_noninfix_targets_in_major
    minor r.capacity_major_out r.capacity_fp_out roots
    r.capacity_fuel_out 1;
  chunked_cheney_promote_head_split_preserves_chain_objects_blue
    minor r.capacity_major_out r.capacity_fp_out roots
    r.capacity_fuel_out;
  ensure_major_head_capacity_preserves_old_major_objects
    major fp alloc_fuel needed fresh;
  chunked_cheney_promote_head_split_preserves_old_major_objects
    minor r.capacity_major_out r.capacity_fp_out roots
    r.capacity_fuel_out;
  let res =
    ChunkedCheney.chunked_cheney_promote
      minor r.capacity_major_out r.capacity_fp_out roots
      r.capacity_fuel_out in
  let old_header_survives (src: obj_addr) (hdr: U64.t)
    : Lemma
        (requires Seq.mem src (MH.major_objects major) /\
                  MH.read_word_in_major major (hd_address src) == Some hdr /\
                  getColor hdr <> Blue /\
                  U64.v (getWosize hdr) >= 1)
        (ensures MH.read_word_in_major res.major_final (hd_address src) ==
                 Some hdr)
    =
    if SpecMajorAlloc.major_fl_head_wosize major fp >= needed then begin
      assert (r.capacity_major_out == major);
      chunked_cheney_promote_head_split_preserves_old_non_blue_header
        minor r.capacity_major_out r.capacity_fp_out roots
        r.capacity_fuel_out src hdr
    end else begin
  ...
  in
  let old_header_survives_for_src (src: obj_addr)
    : Lemma
        (ensures forall (hdr: U64.t).
          Seq.mem src (MH.major_objects major) /\
          MH.read_word_in_major major (hd_address src) == Some hdr /\
  ...
  let old_survives (src: obj_addr)
    ...
    =
    assert (Seq.mem src (MH.major_objects r.capacity_major_out));
    assert (Seq.mem src (MH.major_objects res.major_final))
  in
  FStar.Classical.forall_intro (FStar.Classical.move_requires old_survives);
  assert (GenInv.chunked_major_alloc_shape
            res.major_final res.fp_final r.capacity_fuel_out);
  assert (SpecMajorAlloc.major_fl_chain_terminates
            res.major_final res.fp_final r.capacity_fuel_out = true);
  assert (GenInv.chunked_chain_objects_blue
            res.major_final res.fp_final r.capacity_fuel_out);
  assert (chunked_fwd_targets_above_minor res.fwd_map);
  assert (chunked_fwd_targets_valid_addr res.fwd_map);
  assert (SpecMajorAlloc.major_fl_head_wosize
            res.major_final res.fp_final >= 1)
#pop-options
...
let rec chunked_update_major_pointers_preserves_alloc_shape_aux
  ...
  if alloc_fuel = 0 then begin
    ...
    assert (alloc_fuel <> 0);
    nat_nonzero_gt_zero alloc_fuel;
    assert (alloc_fuel > 0);
  ...
  end else if alloc_fuel > 0 then begin
    assert (alloc_fuel > 0);
    assert (fp <> 0UL);
...
let chunked_cheney_collect_after_minor_promotion_head_preflight
  ...
  assert (chunked_fwd_targets_above_minor prom.fwd_map);
  assert (chunked_fwd_targets_above_minor collect.cmc_fwd);
  assert (chunked_fwd_targets_valid_addr prom.fwd_map);
  assert (chunked_fwd_targets_valid_addr collect.cmc_fwd);
  assert (chunked_fwd_noninfix_targets_in_major
            minor prom.fwd_map prom.major_final);
  assert (GenInv.chunked_major_alloc_shape
            prom.major_final prom.fp_final r.capacity_fuel_out);
  assert (SpecMajorAlloc.major_fl_chain_terminates
            prom.major_final prom.fp_final r.capacity_fuel_out = true);
  assert (GenInv.chunked_chain_objects_blue
            prom.major_final prom.fp_final r.capacity_fuel_out);
  chunked_update_major_pointers_preserves_alloc_shape
    prom.major_final prom.fp_final r.capacity_fuel_out prom.fwd_map;
  GenInv.chunked_major_alloc_shape_elim
    prom.major_final prom.fp_final r.capacity_fuel_out;
  ChunkedUpdate.chunked_update_major_pointers_preserves_wf_and_major_objects
    prom.major_final prom.fwd_map;
  assert (MH.major_objects collect.cmc_major ==
          MH.major_objects prom.major_final);
  reveal_opaque (`%chunked_fwd_noninfix_targets_in_major)
    (chunked_fwd_noninfix_targets_in_major
      minor collect.cmc_fwd collect.cmc_major);
  let noninfix_target (x: U64.t)
    : Lemma
        (requires collect.cmc_fwd x <> 0UL /\
                  ~(is_infix_in_minor minor x))
        (ensures
          U64.v (collect.cmc_fwd x) >= U64.v mword /\
          U64.v (collect.cmc_fwd x) < heap_size /\
          U64.v (collect.cmc_fwd x) % U64.v mword == 0 /\
          Seq.mem ((collect.cmc_fwd x) <: obj_addr)
            (MH.major_objects collect.cmc_major))
    =
    assert (collect.cmc_fwd == prom.fwd_map);
    chunked_fwd_noninfix_targets_in_major_elim
      minor prom.fwd_map prom.major_final x;
    is_val_addr_spec (collect.cmc_fwd x);
    assert (Seq.mem ((collect.cmc_fwd x) <: obj_addr)
              (MH.major_objects prom.major_final));
    assert (Seq.mem ((collect.cmc_fwd x) <: obj_addr)
              (MH.major_objects collect.cmc_major))
  in
  FStar.Classical.forall_intro
    (FStar.Classical.move_requires noninfix_target);
  assert (chunked_fwd_noninfix_targets_in_major
            minor collect.cmc_fwd collect.cmc_major);
  assert (GenInv.chunked_major_alloc_shape
            collect.cmc_major collect.cmc_fp r.capacity_fuel_out);
  ...
  let old_header_survives (src: obj_addr) (hdr: U64.t)
    : Lemma
        (requires Seq.mem src (MH.major_objects major) /\
                  ...
                  getColor hdr <> Blue /\
                  U64.v (getWosize hdr) >= 1)
        (ensures MH.read_word_in_major collect.cmc_major (hd_address src) ==
                 Some hdr)
    =
    ...
    ChunkedUpdate.chunked_update_major_pointers_preserves_header
      prom.major_final prom.fwd_map src hdr;
    assert (MH.read_word_in_major collect.cmc_major (hd_address src) ==
            Some hdr)
  in
  let old_header_survives_for_src (src: obj_addr)
    : Lemma
  ...
  let old_field_effect_stable
    ...
    chunked_fwd_targets_above_minor_expected_stable prom.fwd_map old;
    chunked_fwd_targets_above_minor_expected_stable collect.cmc_fwd old;
    assert (ChunkedUpdate.chunked_update_value_stable prom.fwd_map
              (ChunkedUpdate.chunked_update_expected_value
                prom.fwd_map old));
    assert (ChunkedUpdate.chunked_update_value_stable collect.cmc_fwd
              (ChunkedUpdate.chunked_update_expected_value
                collect.cmc_fwd old));
  ...
  let old_survives (src: obj_addr)
    : Lemma
        (requires Seq.mem src (MH.major_objects major))
        (ensures Seq.mem src (MH.major_objects collect.cmc_major))
    =
    assert (Seq.mem src (MH.major_objects prom.major_final));
