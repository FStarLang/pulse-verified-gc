...
private let chunked_update_nonforwarded_major_field_edge_core
  (minor: minor_state) (mh: MH.major_heap) (fwd: forwarding_map)
  (src expected: obj_addr) (hdr: U64.t) (j: nat)
  (field_addr: hp_addr) (old: U64.t)
  : Lemma
  ...
  let updated = ChunkedUpdate.chunked_update_major_pointers mh fwd in
  ChunkedUpdate.chunked_update_expected_value_effect fwd old;
  assert (ChunkedUpdate.chunked_update_expected_value fwd old == old);
  ChunkedUpdate.chunked_update_value_stable_intro fwd old;
  assert (ChunkedUpdate.chunked_update_value_stable fwd
...
#push-options "--split_queries always --z3rlimit 1 --fuel 1 --ifuel 0"
let chunked_cheney_gc_correct_after_preflight_minor_successor_major_edge_no_field_addr
  (minor: minor_state) (major: MH.major_heap) (fp: U64.t)
  (roots: seq U64.t) (alloc_fuel: nat) (fresh: MH.heap_chunk)
  (src dst: U64.t) (j: nat)
  : Lemma
  ...
  | Some hdr ->
    ...
    | None -> assert False
    | Some field_addr ->
      CG.chunked_major_field_slot_elim target j field_addr;
      assert (U64.v field_addr ==
...
#push-options "--split_queries always --z3rlimit 5 --fuel 1 --ifuel 0"
let chunked_cheney_gc_correct_after_preflight_minor_successor_edge_maps_to_major_edge
  (minor: minor_state) (major: MH.major_heap) (fp: U64.t)
  (roots: seq U64.t) (alloc_fuel: nat) (fresh: MH.heap_chunk)
  (src dst: U64.t) (j: nat) (field_addr: hp_addr)
  : Lemma
...
#push-options "--split_queries always --z3rlimit 1 --fuel 1 --ifuel 0"
let chunked_cheney_gc_correct_after_preflight_minor_successor_edge_maps_to_major_edge_no_field_addr
  (minor: minor_state) (major: MH.major_heap) (fp: U64.t)
  (roots: seq U64.t) (alloc_fuel: nat) (fresh: MH.heap_chunk)
  (src dst: U64.t) (j: nat)
  : Lemma
  ...
  chunked_cheney_gc_correct_after_preflight_minor_successor_forwarded
    minor major fp roots alloc_fuel fresh src dst j;
  chunked_cheney_gc_correct_after_preflight_minor_successor_major_edge_no_field_addr
    minor major fp roots alloc_fuel fresh src dst j
#pop-options

#push-options "--split_queries always --z3rlimit 5 --fuel 1 --ifuel 0"
let chunked_cheney_gc_correct_after_preflight_minor_graph_edge_maps_to_major_edge
  (minor: minor_state) (major: MH.major_heap) (fp: U64.t)
  (roots: seq U64.t) (alloc_fuel: nat) (fresh: MH.heap_chunk)
  (src dst: U64.t)
  : Lemma
    ...
    FStar.IndefiniteDescription.indefinite_description_ghost nat
      (fun j ->
        ...
        CG.chunked_classify_minor_field
          minor major (minor_read_field minor src j) == Some (CG.MinorV dst)) in
  CG.chunked_classify_minor_field_inv_minor
    minor major (minor_read_field minor src j) dst;
  ...
  chunked_cheney_gc_correct_after_preflight_minor_successor_edge_maps_to_major_edge_no_field_addr
    minor major fp roots alloc_fuel fresh src dst j
#pop-options

#push-options "--split_queries always --z3rlimit 5 --fuel 1 --ifuel 0"
let chunked_cheney_gc_correct_after_preflight_minor_major_graph_edge_maps_to_major_edge
  (minor: minor_state) (major: MH.major_heap) (fp: U64.t)
  (roots: seq U64.t) (alloc_fuel: nat) (fresh: MH.heap_chunk)
  ...
  let j =
    FStar.IndefiniteDescription.indefinite_description_ghost nat
      (fun j ->
        j < minor_wosize minor src /\
        CG.chunked_classify_minor_field
          minor major (minor_read_field minor src j) == Some (CG.MajorV dst)) in
  CG.chunked_classify_minor_field_inv_major
    minor major (minor_read_field minor src j) dst;
  assert (j < minor_wosize minor src);
  assert (minor_read_field minor src j == dst);
  assert (Seq.mem dst (MH.major_objects major));
  CheneyPres.chunked_cheney_promote_after_minor_promotion_head_preflight
    minor major fp roots alloc_fuel fresh;
  ChunkedCheney.chunked_cheney_collect_spec_equation
    minor r.capacity_major_out r.capacity_fp_out roots
  ...
  assert (forall (old: obj_addr).
          ...
          Seq.mem old (MH.major_objects prom.major_final));
  assert (Seq.mem dst (MH.major_objects prom.major_final));
  CheneyPres.chunked_cheney_promote_fwd_target_header_matches_minor
    minor r.capacity_major_out r.capacity_fp_out roots
    r.capacity_fuel_out 1 src;
  assert (GenInv.chunked_major_alloc_shape
  ...
  | Some hdr ->
    ...
    | Some field_addr ->
      ...
      CheneyPres.chunked_cheney_promote_fwd_target_fields_match
        minor r.capacity_major_out r.capacity_fp_out roots
        r.capacity_fuel_out 1 src j field_addr;
      assert (U64.v (GC.Spec.Object.getTag hdr) == minor_tag minor src);
      assert (U64.v (GC.Spec.Object.getTag hdr) <
              U64.v GC.Spec.Object.no_scan_tag);
      assert (MH.read_word_in_major prom.major_final field_addr ==
              Some (minor_read_field minor src j));
      assert (MH.read_word_in_major prom.major_final field_addr ==
              Some dst);
      ChunkedUpdate.chunked_update_expected_value_effect prom.fwd_map dst;
      ChunkedUpdate.chunked_update_value_stable_intro prom.fwd_map dst;
      chunked_update_nonforwarded_major_field_edge_core
        minor prom.major_final prom.fwd_map target dst hdr j field_addr dst
#pop-options
...
private let chunked_expansion_value_safety
  (major: MH.major_heap) (fresh: MH.heap_chunk) : prop =
  MH.chunk_disjoint_from_all fresh major /\
  (forall (obj:obj_addr).
...
[@@"opaque_to_smt"]
private let chunked_preflight_expansion_safe_from_value_safety
  (minor: minor_state) (major: MH.major_heap) (fp: U64.t)
  (alloc_fuel: nat) (fresh: MH.heap_chunk)
  : Lemma
    (requires
      GenInv.chunked_collection_heap_shape minor major fp alloc_fuel /\
      SpecMajorAlloc.major_fl_head_wosize major fp <
        PromotionDemand.minor_promotion_demand minor + 1 /\
      chunked_expansion_value_safety major fresh)
    (ensures
      CG.chunked_all_major_object_expansion_safe
        major fresh (MH.major_objects major) 0)
  =
...
let chunked_cheney_gc_correct_after_preflight_value_safety
  (minor: minor_state) (major: MH.major_heap) (fp: U64.t)
  (roots: seq U64.t) (alloc_fuel: nat) (fresh: MH.heap_chunk)
  : Lemma
    (requires
  ...
  if SpecMajorAlloc.major_fl_head_wosize major fp < needed then begin
    ...
    assert (SpecMajorAlloc.major_fl_head_wosize major fp <
            PromotionDemand.minor_promotion_demand minor + 1);
    assert (GenInv.chunked_collection_heap_shape minor major fp alloc_fuel);
    GenInv.chunked_collection_heap_shape_elim minor major fp alloc_fuel;
    GenInv.chunked_major_alloc_shape_elim major fp alloc_fuel;
    assert (MH.well_formed_major_heap major);
    assert (SpecMajorAlloc.major_fl_valid major fp alloc_fuel);
    assert (SpecMajorAlloc.major_fl_blocks_fit major fp alloc_fuel);
    ...
    assert (forall (obj:obj_addr).
      Seq.mem obj (MH.major_objects major) ==>
        CG.chunked_major_field_values_miss_fresh
          major fresh obj (CG.chunked_wosize_nat_of_object major obj) 0);
    assert (chunked_expansion_value_safety major fresh);
    chunked_preflight_expansion_safe_from_value_safety
      minor major fp alloc_fuel fresh
  end;
...
#push-options "--split_queries always --z3rlimit 10 --fuel 1 --ifuel 0"
let chunked_cheney_gc_correct_after_preflight_old_major_major_graph_edge_maps_to_major_edge
  (minor: minor_state) (major: MH.major_heap) (fp: U64.t)
  (roots: seq U64.t) (alloc_fuel: nat) (fresh: MH.heap_chunk)
  ...
  let i =
    FStar.IndefiniteDescription.indefinite_description_ghost nat
      (fun i ->
        ...
        (exists (field_addr: hp_addr). exists (old: U64.t).
          CG.chunked_major_field_slot src i == Some field_addr /\
          MH.read_word_in_major major field_addr == Some old /\
          CG.chunked_classify_major_field minor major old == Some (CG.MajorV dst))) in
  let field_addr =
  ...
  let old =
    FStar.IndefiniteDescription.indefinite_description_ghost U64.t
      (fun old ->
        ...
        MH.read_word_in_major major field_addr == Some old /\
        CG.chunked_classify_major_field minor major old == Some (CG.MajorV dst)) in
  assert (i < CG.chunked_wosize_nat_of_object major src);
  CG.chunked_wosize_nat_header major src hdr;
  assert (i < U64.v (GC.Spec.Object.getWosize hdr));
  CG.chunked_is_no_scan_header major src hdr;
  assert (U64.v (GC.Spec.Object.getTag hdr) <
          U64.v GC.Spec.Object.no_scan_tag);
  assert (CG.chunked_major_field_slot src i == Some field_addr);
  CG.chunked_major_field_slot_elim src i field_addr;
  assert (U64.v field_addr == U64.v src + i * U64.v mword);
  assert (MH.read_word_in_major major field_addr == Some old);
  assert_norm (U64.v mword == 8);
  assert (U64.v mword <> 0);
  CG.chunked_classify_major_field_inv_major minor major old dst;
  assert (old == dst);
  assert (is_val_addr old);
  is_val_addr_spec old;
  assert (U64.v old >= 0);
  assert (U64.v old >= U64.v mword);
  assert (U64.v old < heap_size);
  assert (U64.v old % U64.v mword == 0);
  assert (Seq.mem dst (MH.major_objects major));
  assert (~(is_minor_pointer (to_minor_offset old) /\
            collect.cmc_fwd (to_minor_offset old) <> 0UL));
  chunked_cheney_gc_correct_after_preflight_old_major_nonforwarded_field_edge
    minor major fp roots alloc_fuel fresh src dst hdr i field_addr old
#pop-options

#push-options "--split_queries always --z3rlimit 10 --fuel 1 --ifuel 0"
let chunked_cheney_gc_correct_after_preflight_old_major_minor_graph_edge_maps_to_major_edge
  (minor: minor_state) (major: MH.major_heap) (fp: U64.t)
  (roots: seq U64.t) (alloc_fuel: nat) (fresh: MH.heap_chunk)
  ...
  let collect =
    ChunkedCheney.chunked_cheney_collect_spec
      minor r.capacity_major_out r.capacity_fp_out roots
      r.capacity_fuel_out in
  CG.chunked_major_edge_elim minor major src (CG.MinorV dst);
  assert (Seq.mem src (MH.major_objects major));
  assert (CG.chunked_is_no_scan major src == false);
  let i =
    FStar.IndefiniteDescription.indefinite_description_ghost nat
      (fun i ->
        ...
        (exists (field_addr: hp_addr). exists (old: U64.t).
          CG.chunked_major_field_slot src i == Some field_addr /\
          MH.read_word_in_major major field_addr == Some old /\
          CG.chunked_classify_major_field minor major old == Some (CG.MinorV dst))) in
  let field_addr =
    FStar.IndefiniteDescription.indefinite_description_ghost hp_addr
      (fun field_addr ->
        exists (old: U64.t).
  ...
  assert (U64.v field_addr == U64.v src + i * U64.v mword);
  assert (MH.read_word_in_major major field_addr == Some old);
  CG.chunked_classify_major_field_inv_minor minor major old dst;
  assert (to_minor_offset old == dst);
  assert (is_minor_pointer dst);
  assert (Seq.mem dst (minor_objects minor));
  chunked_cheney_gc_correct_after_preflight_forwarded_minor_object_in_major
  ...
  is_val_addr_spec (collect.cmc_fwd dst);
  assert_norm (U64.v mword == 8);
  assert (U64.v mword <> 0);
  assert (U64.v mword >= 0);
  assert (U64.v (collect.cmc_fwd dst) >= 0);
  assert (U64.v (collect.cmc_fwd dst) >= U64.v mword);
  assert (U64.v (collect.cmc_fwd dst) < heap_size);
  assert (U64.v (collect.cmc_fwd dst) % U64.v mword == 0);
  let expected : obj_addr = collect.cmc_fwd dst in
  assert (Seq.mem expected (MH.major_objects collect.cmc_major));
  assert (collect.cmc_fwd dst == expected);
...
#push-options "--split_queries always --z3rlimit 5 --fuel 1 --ifuel 0"
let chunked_cheney_gc_correct_after_preflight_graph_edges_map_to_major_edges
  (minor: minor_state) (major: MH.major_heap) (fp: U64.t)
  (roots: seq U64.t) (alloc_fuel: nat) (fresh: MH.heap_chunk)
  : Lemma
    (requires
      ...
      (SpecMajorAlloc.major_fl_head_wosize major fp <
       ...
       major fresh (MH.major_objects major) 0))
    (ensures
      chunked_graph_edges_map_to_major_edges_prop
        minor major fp roots alloc_fuel fresh)
  =
  let prove_for_u (u: CG.combined_vertex)
    : Lemma
          ...
          (let needed = PromotionDemand.minor_promotion_demand minor + 1 in
           ...
           CG.mem_ce
            ...
            (CG.build_chunked_combined_graph
             collect.cmc_minor collect.cmc_major)))
      =
      chunked_cheney_gc_correct_after_preflight_graph_edge_maps_to_major_edge
        minor major fp roots alloc_fuel fresh u v
    in
...
#push-options "--split_queries always --z3rlimit 5 --fuel 1 --ifuel 0"
let chunked_cheney_gc_correct_after_preflight_graph_maps_to_major_graph
  (minor: minor_state) (major: MH.major_heap) (fp: U64.t)
  (roots: seq U64.t) (alloc_fuel: nat) (fresh: MH.heap_chunk)
  : Lemma
    (requires
      ...
      (SpecMajorAlloc.major_fl_head_wosize major fp <
       ...
       major fresh (MH.major_objects major) 0))
    (ensures
      chunked_graph_maps_to_major_graph_prop
        minor major fp roots alloc_fuel fresh)
  =
  chunked_cheney_gc_correct_after_preflight_graph_vertices_map_to_major_vertices
    minor major fp roots alloc_fuel fresh;
  chunked_cheney_gc_correct_after_preflight_graph_edges_map_to_major_edges
    minor major fp roots alloc_fuel fresh
#pop-options

#push-options "--split_queries always --z3rlimit 5 --fuel 1 --ifuel 0"
private let above_minor_target_not_rewritten
  (fwd: forwarding_map) (dst: obj_addr)
  : Lemma
    (requires U64.v dst >= minor_heap_size)
    (ensures
      ~(is_minor_pointer (to_minor_offset dst) /\
        fwd (to_minor_offset dst) <> 0UL))
  =
  to_minor_offset_stable_above_minor dst;
  assert (to_minor_offset dst == dst);
  if is_minor_pointer (to_minor_offset dst) then begin
    assert (is_minor_pointer dst);
    assert (U64.v dst < minor_heap_size);
    assert False
  end
#pop-options

#push-options "--split_queries always --z3rlimit 5 --fuel 1 --ifuel 0"
let chunked_graph_edge_maps_to_major_above_minor_targets_ready_implies_ready
  (minor: minor_state) (major: MH.major_heap) (fp: U64.t)
  (roots: seq U64.t) (alloc_fuel: nat) (fresh: MH.heap_chunk)
  (u v: CG.combined_vertex)
  : Lemma
    (requires
      chunked_graph_edge_maps_to_major_above_minor_targets_ready
        minor major fp roots alloc_fuel fresh u v)
    (ensures
      chunked_graph_edge_maps_to_major_ready
        minor major fp roots alloc_fuel fresh u v)
  ...
  | CG.MinorV src, CG.MajorV dst ->
    let dst_obj =
      FStar.IndefiniteDescription.indefinite_description_ghost obj_addr
        (fun dst_obj ->
          dst_obj == dst /\ U64.v dst_obj >= minor_heap_size) in
    assert (dst_obj == dst);
    assert (U64.v dst_obj >= minor_heap_size);
    above_minor_target_not_rewritten collect.cmc_fwd dst_obj
  | CG.MajorV src, CG.MajorV dst ->
    let dst_obj =
      FStar.IndefiniteDescription.indefinite_description_ghost obj_addr
        (fun dst_obj ->
...
#push-options "--split_queries always --z3rlimit 5 --fuel 1 --ifuel 0"
let chunked_cheney_gc_correct_after_preflight_graph_edge_above_minor_targets_maps_to_major_edge
  (minor: minor_state) (major: MH.major_heap) (fp: U64.t)
  (roots: seq U64.t) (alloc_fuel: nat) (fresh: MH.heap_chunk)
  (u v: CG.combined_vertex)
  : Lemma
      ...
      (let needed = PromotionDemand.minor_promotion_demand minor + 1 in
       ...
       CG.mem_ce
        ...
        (CG.build_chunked_combined_graph
         collect.cmc_minor collect.cmc_major)))
  =
  chunked_graph_edge_maps_to_major_above_minor_targets_ready_implies_ready
    minor major fp roots alloc_fuel fresh u v;
  chunked_cheney_gc_correct_after_preflight_graph_edge_maps_to_major_edge
    minor major fp roots alloc_fuel fresh u v
#pop-options
...
#push-options "--split_queries always --z3rlimit 5 --fuel 1 --ifuel 0"
let chunked_cheney_gc_correct_after_preflight_graph_edges_above_minor_targets_map_to_major_edges
  (minor: minor_state) (major: MH.major_heap) (fp: U64.t)
  (roots: seq U64.t) (alloc_fuel: nat) (fresh: MH.heap_chunk)
  : Lemma
    (requires
      ...
      (SpecMajorAlloc.major_fl_head_wosize major fp <
       ...
       major fresh (MH.major_objects major) 0))
    (ensures
      chunked_graph_edges_above_minor_targets_map_to_major_edges_prop
        minor major fp roots alloc_fuel fresh)
  =
  let prove_for_u (u: CG.combined_vertex)
    : Lemma
          ...
          (let needed = PromotionDemand.minor_promotion_demand minor + 1 in
           ...
           CG.mem_ce
            ...
            (CG.build_chunked_combined_graph
             collect.cmc_minor collect.cmc_major)))
      =
      chunked_cheney_gc_correct_after_preflight_graph_edge_above_minor_targets_maps_to_major_edge
        minor major fp roots alloc_fuel fresh u v
    in
    FStar.Classical.forall_intro (FStar.Classical.move_requires prove_for_v)
  in
...
#push-options "--split_queries always --z3rlimit 5 --fuel 1 --ifuel 0"
private let nonblue_major_source_header_witness
  (major: MH.major_heap) (src: obj_addr)
  : Lemma
    (requires
  ...
  match MH.read_word_in_major major (GC.Spec.Heap.hd_address src) with
  | None -> assert False
  | Some hdr ->
    GenInv.chunked_is_blue_header major src hdr;
    assert (GenInv.chunked_is_blue major src ==
            (GC.Spec.Object.getColor hdr = GC.Lib.Header.Blue));
    if GC.Spec.Object.getColor hdr = GC.Lib.Header.Blue then
      assert False
#pop-options

#push-options "--split_queries always --z3rlimit 5 --fuel 1 --ifuel 0"
let chunked_graph_edge_maps_to_major_nonblue_sources_above_minor_targets_ready_implies_above_minor_targets_ready
  (minor: minor_state) (major: MH.major_heap) (fp: U64.t)
  (roots: seq U64.t) (alloc_fuel: nat) (fresh: MH.heap_chunk)
  (u v: CG.combined_vertex)
  : Lemma
    (requires
      MH.well_formed_major_heap major /\
      chunked_graph_edge_maps_to_major_nonblue_sources_above_minor_targets_ready
        minor major fp roots alloc_fuel fresh u v)
    (ensures
      chunked_graph_edge_maps_to_major_above_minor_targets_ready
        minor major fp roots alloc_fuel fresh u v)
  ...
  | CG.MajorV src, CG.MajorV dst ->
    ...
    nonblue_major_source_header_witness major src_obj;
    let hdr =
      FStar.IndefiniteDescription.indefinite_description_ghost U64.t
        (fun hdr ->
          MH.read_word_in_major major (GC.Spec.Heap.hd_address src_obj) ==
            Some hdr /\
          GC.Spec.Object.getColor hdr <> GC.Lib.Header.Blue) in
    assert (MH.read_word_in_major major (GC.Spec.Heap.hd_address src_obj) ==
            Some hdr);
    assert (GC.Spec.Object.getColor hdr <> GC.Lib.Header.Blue)
  | CG.MajorV src, CG.MinorV dst ->
    let src_obj =
...
#push-options "--split_queries always --z3rlimit 5 --fuel 1 --ifuel 0"
let chunked_cheney_gc_correct_after_preflight_graph_edge_nonblue_sources_above_minor_targets_maps_to_major_edge
  (minor: minor_state) (major: MH.major_heap) (fp: U64.t)
  (roots: seq U64.t) (alloc_fuel: nat) (fresh: MH.heap_chunk)
  (u v: CG.combined_vertex)
  : Lemma
  ...
  GenInv.chunked_major_alloc_shape_elim major fp alloc_fuel;
  assert (MH.well_formed_major_heap major);
  chunked_graph_edge_maps_to_major_nonblue_sources_above_minor_targets_ready_implies_above_minor_targets_ready
    minor major fp roots alloc_fuel fresh u v;
  chunked_cheney_gc_correct_after_preflight_graph_edge_above_minor_targets_maps_to_major_edge
    minor major fp roots alloc_fuel fresh u v
#pop-options
...
#push-options "--split_queries always --z3rlimit 5 --fuel 1 --ifuel 0"
let chunked_cheney_gc_correct_after_preflight_graph_edges_nonblue_sources_above_minor_targets_map_to_major_edges
  (minor: minor_state) (major: MH.major_heap) (fp: U64.t)
  (roots: seq U64.t) (alloc_fuel: nat) (fresh: MH.heap_chunk)
  : Lemma
    (requires
      ...
      (SpecMajorAlloc.major_fl_head_wosize major fp <
       ...
       major fresh (MH.major_objects major) 0))
    (ensures
      chunked_graph_edges_nonblue_sources_above_minor_targets_map_to_major_edges_prop
        minor major fp roots alloc_fuel fresh)
  =
  let prove_for_u (u: CG.combined_vertex)
    : Lemma
          ...
          (let needed = PromotionDemand.minor_promotion_demand minor + 1 in
           ...
           CG.mem_ce
            ...
            (CG.build_chunked_combined_graph
             collect.cmc_minor collect.cmc_major)))
      =
      chunked_cheney_gc_correct_after_preflight_graph_edge_nonblue_sources_above_minor_targets_maps_to_major_edge
        minor major fp roots alloc_fuel fresh u v
    in
    FStar.Classical.forall_intro (FStar.Classical.move_requires prove_for_v)
  in
