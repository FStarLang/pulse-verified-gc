...
private let chunked_old_raw_target_transferred_to_updated
  (mh post: MH.major_heap)
  (src: obj_addr)
  (idx: nat)
  (field_addr: hp_addr)
  (old raw: U64.t)
  : Lemma
      (requires
        RangePres.same_chunk_ranges mh post /\
        MH.major_objects post == MH.major_objects mh /\
        chunked_major_raw_field_targets_in_major mh /\
        Seq.mem src (MH.major_objects mh) /\
        idx < CG.chunked_wosize_nat_of_object mh src /\
        CG.chunked_major_field_slot src idx == Some field_addr /\
        ...
        raw == old /\
        MarkDefs.chunked_is_pointer_field post raw)
      (ensures
        Seq.mem (MarkDefs.chunked_pointer_field_as_obj_addr post raw)
          (MH.major_objects post))
  =
  MarkDefs.chunked_is_pointer_field_step post raw;
  assert (MH.is_major_pointer post raw);
  RangePres.same_chunk_ranges_preserves_is_major_pointer mh post old;
  assert (MH.is_major_pointer mh old);
  MarkDefs.chunked_is_pointer_field_step mh old;
  assert (MarkDefs.chunked_is_pointer_field mh old);
  chunked_major_raw_field_targets_in_major_elim
    mh src idx field_addr old;
  MarkDefs.chunked_pointer_field_as_obj_addr_step mh old;
  MarkDefs.chunked_pointer_field_as_obj_addr_step post raw;
  assert (MarkDefs.chunked_pointer_field_as_obj_addr mh old == old);
  assert (MarkDefs.chunked_pointer_field_as_obj_addr post raw == raw);
  assert (MarkDefs.chunked_pointer_field_as_obj_addr mh old ==
          MarkDefs.chunked_pointer_field_as_obj_addr post raw);
  assert (Seq.mem (MarkDefs.chunked_pointer_field_as_obj_addr post raw)
            (MH.major_objects mh));
  assert (Seq.mem (MarkDefs.chunked_pointer_field_as_obj_addr post raw)
            (MH.major_objects post))

#push-options "--z3rlimit 10 --fuel 1 --ifuel 1"
let chunked_major_raw_field_targets_in_major_preserved_by_update
  (minor: minor_state)
  (mh: MH.major_heap)
  (fwd: forwarding_map)
  : Lemma
    ...
    : Lemma
        ...
        (ensures
          ...
          Seq.mem (MarkDefs.chunked_pointer_field_as_obj_addr post raw)
            (MH.major_objects post))
    =
    if MarkDefs.chunked_is_pointer_field post raw then begin
      assert (Seq.mem src (MH.major_objects mh));
      MH.major_objects_member_header_read_some mh src;
      ...
      | Some hdr ->
        ...
        | Some old ->
          ...
          if getColor hdr = Header.Blue then begin
            ...
            assert (raw == old);
            chunked_old_raw_target_transferred_to_updated
              mh post src idx field_addr old raw
            ...
            if is_minor_pointer old_val then begin
              ...
              if new_val <> 0UL then begin
                ...
                MarkDefs.chunked_pointer_field_as_obj_addr_step post raw;
                assert (raw == new_val);
                assert (MarkDefs.chunked_pointer_field_as_obj_addr post raw ==
                        raw);
                assert (is_val_addr new_val);
                assert (Seq.mem ((new_val) <: obj_addr)
                          (MH.major_objects post));
                assert (MarkDefs.chunked_pointer_field_as_obj_addr post raw ==
                        ((new_val) <: obj_addr))
              end else begin
  ...
  FStar.Classical.forall_intro_4
    (FStar.Classical.move_requires_4 one);
  chunked_major_raw_field_targets_in_major_intro post
#pop-options

#pop-options

[@"opaque_to_smt"]
let chunked_fwd_noninfix_targets_not_infix
  (minor: minor_state)
  (fwd: forwarding_map)
  (mh: MH.major_heap)
  : prop =
...
let chunked_fwd_noninfix_targets_not_infix_intro
  (minor: minor_state)
  (fwd: forwarding_map)
  (mh: MH.major_heap)
  : Lemma
  ...
  =
  reveal_opaque (`%chunked_fwd_noninfix_targets_not_infix)
    (chunked_fwd_noninfix_targets_not_infix minor fwd mh)

let chunked_fwd_noninfix_targets_not_infix_elim
  (minor: minor_state)
  (fwd: forwarding_map)
  (mh: MH.major_heap)
  (x: U64.t)
...
private let chunked_update_preserves_not_infix_of_member
  (mh: MH.major_heap)
  (fwd: forwarding_map)
  (target: obj_addr)
  : Lemma
      ...
      (ensures
        ~(SweepDefs.chunked_is_infix
          (ChunkedUpdate.chunked_update_major_pointers mh fwd) target))
  =
  ...
  | Some hdr ->
    ...
    SweepDefs.chunked_tag_of_object_some post target hdr;
...
#push-options "--z3rlimit 10 --fuel 1 --ifuel 1 --split_queries always"
let chunked_nonblue_scanned_raw_targets_in_major_preserved_by_update
  (minor: minor_state)
  (mh: MH.major_heap)
  (fwd: forwarding_map)
  : Lemma
  ...
  let old_target_transferred
    ...
    : Lemma
        (requires
          ...
          ~(MarkDefs.chunked_is_no_scan mh src) /\
          U64.v i <= U64.v (SweepDefs.chunked_wosize_of_object mh src) /\
          CG.chunked_major_field_slot src idx == Some field_addr /\
          MH.read_word_in_major mh field_addr == Some old /\
          expected == old /\
          MarkDefs.chunked_is_pointer_field post expected)
        (ensures
          (let child_raw =
             MarkDefs.chunked_pointer_field_as_obj_addr post expected in
           Seq.mem child_raw (MH.major_objects post) /\
           ~(SweepDefs.chunked_is_infix post child_raw)))
    =
    MH.major_objects_member_header_read_some mh src;
    (match MH.read_word_in_major mh (hd_address src) with
    | None -> assert False
    | Some hdr ->
      ...
      assert (idx < U64.v (getWosize hdr));
      assert (idx < CG.chunked_wosize_nat_of_object mh src));
    chunked_get_field_from_major_field_slot mh src i idx field_addr old;
    assert (MarkDefs.chunked_get_field mh src i == old);
    MarkDefs.chunked_is_pointer_field_step post expected;
    RangePres.same_chunk_ranges_preserves_is_major_pointer mh post old;
    MarkDefs.chunked_is_pointer_field_step mh old;
    assert (MarkDefs.chunked_is_pointer_field mh old);
    ChunkedMarkTargetMembership
      .chunked_nonblue_scanned_raw_targets_in_major_elim mh src i;
    MarkDefs.chunked_pointer_field_as_obj_addr_step mh old;
    MarkDefs.chunked_pointer_field_as_obj_addr_step post expected;
    let child : obj_addr =
      MarkDefs.chunked_pointer_field_as_obj_addr mh old in
    assert (Seq.mem child (MH.major_objects mh));
    assert (~(SweepDefs.chunked_is_infix mh child));
    assert (Seq.mem child (MH.major_objects post));
    chunked_update_preserves_not_infix_of_member mh fwd child;
    assert (child == MarkDefs.chunked_pointer_field_as_obj_addr post expected)
  in
  let one
    (src: obj_addr)
    (i: U64.t{U64.v i >= 1})
    : Lemma
        (requires
          Seq.mem src (MH.major_objects post) /\
          ~(SweepDefs.chunked_is_blue post src) /\
          ~(MarkDefs.chunked_is_no_scan post src) /\
          U64.v i <= U64.v (SweepDefs.chunked_wosize_of_object post src))
        (ensures
          (let v = MarkDefs.chunked_get_field post src i in
           if MarkDefs.chunked_is_pointer_field post v then
             let child_raw =
               MarkDefs.chunked_pointer_field_as_obj_addr post v in
             Seq.mem child_raw (MH.major_objects post) /\
             ~(SweepDefs.chunked_is_infix post child_raw)
           else
             True))
    ...
    | Some hdr ->
      ChunkedUpdate.chunked_update_major_pointers_preserves_header
        mh fwd src hdr;
      assert (MH.read_word_in_major post (hd_address src) == Some hdr);
      SweepDefs.chunked_read_header_step post src;
      assert (SweepDefs.chunked_read_header post src == Some hdr);
      SweepDefs.chunked_wosize_of_object_some post src hdr;
      CG.chunked_wosize_nat_header mh src hdr;
      CG.chunked_wosize_nat_header post src hdr;
      assert (U64.v i <= U64.v (getWosize hdr));
      assert (idx < U64.v (getWosize hdr));
      SweepDefs.chunked_is_blue_header post src hdr;
      assert (getColor hdr <> Header.Blue);
      SweepDefs.chunked_read_header_step mh src;
      assert (SweepDefs.chunked_read_header mh src == Some hdr);
      SweepDefs.chunked_color_of_object_some mh src hdr;
      SweepDefs.chunked_is_blue_header mh src hdr;
      assert (~(SweepDefs.chunked_is_blue mh src));
      MarkDefs.chunked_is_no_scan_step post src;
      SweepDefs.chunked_tag_of_object_some post src hdr;
      assert (U64.v (getTag hdr) < U64.v no_scan_tag);
      MarkDefs.chunked_is_no_scan_step mh src;
      SweepDefs.chunked_tag_of_object_some mh src hdr;
      SweepDefs.chunked_wosize_of_object_some mh src hdr;
      assert (U64.v i <=
        U64.v (SweepDefs.chunked_wosize_of_object mh src));
      assert (~(MarkDefs.chunked_is_no_scan mh src));
      CG.chunked_major_field_slot_of_object_header mh src hdr idx;
      match CG.chunked_major_field_slot src idx with
      | None -> assert False
      | Some field_addr ->
        ...
        | Some old ->
          ...
          chunked_get_field_from_major_field_slot
            post src i idx field_addr expected;
          assert (MarkDefs.chunked_get_field post src i == expected);
          if MarkDefs.chunked_is_pointer_field post expected then begin
            ChunkedUpdate.chunked_update_expected_value_effect fwd old;
            let old_val = to_minor_offset old in
            if is_minor_pointer old_val then begin
              let new_val = fwd old_val in
              if new_val <> 0UL then begin
                ...
                assert (Seq.mem child (MH.major_objects post));
                assert (child ==
                  MarkDefs.chunked_pointer_field_as_obj_addr post expected);
                chunked_fwd_noninfix_targets_not_infix_elim
                  minor fwd post old_val child
              end else begin
                assert (U64.v i <=
                  U64.v (SweepDefs.chunked_wosize_of_object mh src));
                old_target_transferred src i idx field_addr old expected
              end
            end else begin
              assert (U64.v i <=
                U64.v (SweepDefs.chunked_wosize_of_object mh src));
  ...
  ChunkedMarkTargetMembership
    .chunked_nonblue_scanned_raw_targets_in_major_intro post
...
let chunked_major_gc_bounded_after_gray_roots_nonblue_static_raw_target_policy
  (mh: MH.major_heap)
  (roots: Seq.seq obj_addr)
  (cap: nat{cap > 0})
  (mark_fuel: nat)
...
let chunked_major_gc_bounded_after_gray_roots_nonblue_static_raw_target_policy_intro
  (mh: MH.major_heap)
  (roots: Seq.seq obj_addr)
  (cap: nat{cap > 0})
  (mark_fuel: nat)
  : Lemma
      (requires
        ...
        mark_fuel >= Seq.length (MH.major_objects mh))
      (ensures
        chunked_major_gc_bounded_after_gray_roots_nonblue_static_raw_target_policy
          mh roots cap mark_fuel)
  =
  ()

let chunked_major_gc_bounded_after_gray_roots_nonblue_static_raw_target_policy_from_pre_gray
  (mh: MH.major_heap)
  (roots: Seq.seq obj_addr)
  (cap: nat{cap > 0})
  (mark_fuel: nat)
  : Lemma
      (requires
        ...
        ChunkedMarkTargetMembership.chunked_nonblue_scanned_raw_targets_in_major mh /\
        Seq.length (MH.major_objects mh) <= cap /\
        mark_fuel >= Seq.length (MH.major_objects mh))
      (ensures
        chunked_major_gc_bounded_after_gray_roots_nonblue_static_raw_target_policy
          mh roots cap mark_fuel)
  =
  let roots_sweep_nonblue (root: obj_addr)
    : Lemma
        (requires
          Seq.mem root roots /\
          Seq.mem root (MH.major_objects mh))
    ...
    if SweepDefs.chunked_is_blue mh root then begin
      SweepDefs.chunked_is_blue_read_header mh root;
      SweepDefs.chunked_read_header_step mh root;
      match SweepDefs.chunked_read_header mh root with
      | Some hdr ->
        assert (MH.read_word_in_major mh (hd_address root) == Some hdr);
        GenInv.chunked_is_blue_header mh root hdr;
        assert (GenInv.chunked_is_blue mh root);
        assert False
      | None -> assert False
    end
  in
  FStar.Classical.forall_intro
    (FStar.Classical.move_requires roots_sweep_nonblue);
  ChunkedMarkTargetMembership.chunked_nonblue_scanned_raw_targets_in_major_preserved_by_gray_roots
    mh roots;
  chunked_major_gc_bounded_after_gray_roots_nonblue_static_raw_target_policy_intro
    mh roots cap mark_fuel

let chunked_major_gc_bounded_after_gray_roots_target_membership_policy_from_nonblue_static_raw_targets
  (mh: MH.major_heap)
  (roots: Seq.seq obj_addr)
  (cap: nat{cap > 0})
  (mark_fuel: nat)
  ...
  assert (Seq.length (MH.major_objects grayed) ==
          Seq.length (MH.major_objects mh));
  ChunkedMarkTargetMembership.chunked_mark_bounded_target_membership_policy_from_nonblue_static
    grayed cap mark_fuel;
  chunked_major_gc_bounded_after_gray_roots_target_membership_policy_intro
    mh roots cap mark_fuel

let chunked_major_gc_bounded_after_gray_roots_policy_from_nonblue_static_raw_targets
  (mh: MH.major_heap)
  (roots: Seq.seq obj_addr)
  (cap: nat{cap > 0})
  (mark_fuel: nat)
  : Lemma
      ...
      (ensures
        chunked_major_gc_bounded_after_gray_roots_policy
          mh roots cap mark_fuel)
  =
  chunked_major_gc_bounded_after_gray_roots_target_membership_policy_from_nonblue_static_raw_targets
    mh roots cap mark_fuel;
