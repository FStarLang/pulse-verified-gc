...
let ensure_major_capacity_fl_chain_terminates
  (mh: MH.major_heap) (fp: obj_addr)
  (fuel needed: nat) (fresh: MH.heap_chunk)
  : Lemma
  ...
  = if major_fl_capacity mh fp fuel >= needed then ()
      ...
      expand_major_heap_links_fl_chain_terminates mh fresh fp fuel
...
let ensure_major_head_capacity_fl_chain_terminates
  (mh: MH.major_heap) (fp: U64.t) (fuel: nat) (needed: nat{needed > 0})
  (fresh: MH.heap_chunk)
  : Lemma
      (requires major_fl_valid mh fp fuel /\
...
let major_alloc_result_fp_link_not_self (r: major_alloc_result) : Tot prop =
  if r.major_fp_out = 0UL then False
  else if U64.v r.major_fp_out < U64.v zero_addr + U64.v mword ||
          U64.v r.major_fp_out >= heap_size ||
          U64.v r.major_fp_out % U64.v mword <> 0 then False
  else
    match MH.read_word_in_major r.major_alloc_out (r.major_fp_out <: obj_addr) with
    | Some next -> next <> r.major_fp_out
    | None -> False
...
let objects_in_chunk_from_head_split_preserves_tail_member
  ...
  : Lemma
      (requires requested_wz > 0 /\
                ...
                U64.v rem_hd == U64.v (hd_address obj) + (1 + requested_wz) * U64.v mword /\
                U64.v rem_obj == U64.v rem_hd + U64.v mword /\
                U64.v old_next == U64.v (hd_address obj) + (1 + block_wz) * U64.v mword /\
                U64.v old_next < MH.chunk_end c /\
                U64.v (hd_address obj) + (1 + block_wz) * U64.v mword <=
                  MH.chunk_end c /\
                Seq.mem x (MH.objects_in_chunk_from c old_next))
       (ensures
         (let hd = hd_address obj in
          let alloc_hdr =
            Alloc.make_header (U64.uint_to_t requested_wz) Alloc.white_bits 0UL in
          ...
          let c2 = MH.write_word_in_chunk c1 rem_hd rem_hdr in
          let c3 = MH.write_word_in_chunk c2 rem_obj next_fp in
          Seq.mem x (MH.objects_in_chunk_from c3 hd)))
  =
  let hd = hd_address obj in
  let alloc_hdr =
    Alloc.make_header (U64.uint_to_t requested_wz) Alloc.white_bits 0UL in
  ...
  let c2 = MH.write_word_in_chunk c1 rem_hd rem_hdr in
  let c3 = MH.write_word_in_chunk c2 rem_obj next_fp in
  let rem_wz = U64.v rem_wz_u in
  assert (rem_wz == block_wz - requested_wz - 1);
  assert (rem_wz >= 1);
  assert (U64.v rem_hd ==
  ...
  assert (U64.v old_next ==
          U64.v rem_hd + (rem_wz + 1) * U64.v mword);
  ...
  FStar.Math.Lemmas.lemma_mult_le_right
    (U64.v mword) 1 (rem_wz + 1);
  assert (U64.v mword <= (rem_wz + 1) * U64.v mword);
  ...
  assert (rem_wz + 1 >= 2);
  FStar.Math.Lemmas.lemma_mult_le_right
    (U64.v mword) 2 (rem_wz + 1);
  assert (2 * U64.v mword <= (rem_wz + 1) * U64.v mword);
  ...
  assert (Seq.mem x (MH.objects_in_chunk_from c3 old_next));
  ...
  assert (U64.v rem_hd + (U64.v (Obj.getWosize rem_hdr) + 1) *
            U64.v mword == U64.v old_next);
  MH.objects_in_chunk_from_tail_mem c3 rem_hd old_next x;
  assert (Seq.mem x (MH.objects_in_chunk_from c3 rem_hd));
  AllocHeader.make_header_getWosize
    (U64.uint_to_t requested_wz) Alloc.white_bits 0UL;
  assert (Obj.getWosize alloc_hdr == U64.uint_to_t requested_wz);
  MH.read_write_in_chunk_same c hd alloc_hdr;
  ...
  assert (U64.v hd +
          ...
          U64.v rem_hd);
  MH.objects_in_chunk_from_tail_mem c3 hd rem_hd x;
...
#push-options "--z3rlimit 20 --split_queries always --fuel 0 --ifuel 0"
let head_split_major_preserves_read_at
  (mh: MH.major_heap) (idx: nat) (obj: obj_addr) (addr: hp_addr)
  (old: U64.t) (requested_wz block_wz: nat) (next_fp: U64.t)
  (rem_wz_u: U64.t) (rem_hd: hp_addr) (rem_obj: obj_addr)
  : Lemma
      (requires MH.well_formed_major_heap mh /\
                idx < Seq.length mh /\
       ...
       (ensures
         (let hd = hd_address obj in
          ...
          let mh3 = major_write_word_or_same mh2 rem_obj next_fp in
          if addr = hd then
            MH.read_word_in_major mh3 addr == Some alloc_hdr
          else
            MH.read_word_in_major mh3 addr == Some old))
  =
  FStar.Math.Lemmas.pow2_lt_compat 64 54;
  assert (requested_wz < pow2 64);
  ...
  if MH.word_in_chunk c addr then begin
    well_formed_no_prior_word_in_selected_chunk mh idx addr;
    well_formed_no_prior_contains_selected_chunk mh idx addr;
    MH.read_word_in_major_at_index mh addr idx;
    assert (MH.read_word_in_chunk c addr == old);
    head_split_chunk_preserves_read_at
      c obj addr requested_wz block_wz next_fp rem_wz_u rem_hd rem_obj;
    indexed_chunk_write_preserves_word_no_prior mh idx idx hd addr alloc_hdr;
    indexed_chunk_write_preserves_contains_no_prior mh idx idx hd addr alloc_hdr;
    assert (forall k. k < idx ==> ~(MH.word_in_chunk (Seq.index mh1 k) addr));
    assert (forall k. k < idx ==> ~(MH.chunk_contains_addr (Seq.index mh1 k) addr));
    indexed_chunk_write_preserves_word_no_prior mh1 idx idx rem_hd addr rem_hdr;
    indexed_chunk_write_preserves_contains_no_prior mh1 idx idx rem_hd addr rem_hdr;
    assert (forall k. k < idx ==> ~(MH.word_in_chunk (Seq.index mh2 k) addr));
    assert (forall k. k < idx ==> ~(MH.chunk_contains_addr (Seq.index mh2 k) addr));
    indexed_chunk_write_preserves_word_no_prior mh2 idx idx rem_obj addr next_fp;
    indexed_chunk_write_preserves_contains_no_prior mh2 idx idx rem_obj addr next_fp;
    assert (forall k. k < idx ==> ~(MH.word_in_chunk (Seq.index mh3 k) addr));
    assert (forall k. k < idx ==> ~(MH.chunk_contains_addr (Seq.index mh3 k) addr));
    assert (Seq.index mh3 idx == c3);
    MH.read_word_in_major_at_index mh3 addr idx;
    if addr = hd then
      assert (MH.read_word_in_major mh3 addr == Some alloc_hdr)
    else
      assert (MH.read_word_in_major mh3 addr == Some old)
  end else begin
    assert (~(MH.chunk_contains_addr c addr));
    MH.read_word_in_major_lookup_index mh addr old;
    let target_idx = MH.lookup_chunk_index_value mh addr in
    assert (target_idx < Seq.length mh);
    assert (MH.word_in_chunk (Seq.index mh target_idx) addr);
    assert (MH.read_word_in_chunk (Seq.index mh target_idx) addr == old);
    MH.lookup_chunk_index_some mh addr target_idx;
    assert (forall k. k < target_idx ==> ~(MH.word_in_chunk (Seq.index mh k) addr));
    assert (forall k. k < target_idx ==> ~(MH.chunk_contains_addr (Seq.index mh k) addr));
    if target_idx = idx then begin
      assert (Seq.index mh target_idx == c);
      assert (MH.chunk_contains_addr c addr);
      assert False
    end;
    indexed_chunk_write_preserves_word_no_prior mh idx target_idx hd addr alloc_hdr;
    indexed_chunk_write_preserves_contains_no_prior mh idx target_idx hd addr alloc_hdr;
    indexed_chunk_write_preserves_word_no_prior mh1 idx target_idx rem_hd addr rem_hdr;
    indexed_chunk_write_preserves_contains_no_prior mh1 idx target_idx rem_hd addr rem_hdr;
    indexed_chunk_write_preserves_word_no_prior mh2 idx target_idx rem_obj addr next_fp;
    indexed_chunk_write_preserves_contains_no_prior mh2 idx target_idx rem_obj addr next_fp;
    assert (forall k. k < target_idx ==> ~(MH.chunk_contains_addr (Seq.index mh3 k) addr));
    assert (Seq.index mh1 target_idx == Seq.index mh target_idx);
    assert (Seq.index mh2 target_idx == Seq.index mh target_idx);
    assert (Seq.index mh3 target_idx == Seq.index mh target_idx);
    MH.read_word_in_major_at_index mh3 addr target_idx;
    assert (MH.read_word_in_major mh3 addr == Some old)
  end
#pop-options
...
#push-options "--z3rlimit 20 --split_queries always --fuel 0 --ifuel 0"
let head_split_preserves_free_node_object
  ...
  : Lemma
      (requires MH.well_formed_major_heap mh /\
                ...
                U64.v (Obj.getWosize
                  (MH.read_word_in_chunk (Seq.index mh idx) (hd_address obj))) ==
                  block_wz /\
                requested_wz > 0 /\
                block_wz >= requested_wz /\
                block_wz < pow2 54 /\
       ...
       (ensures
        (let c = Seq.index mh idx in
         ...
         let c3 = MH.write_word_in_chunk c2 rem_obj next_fp in
         let mh3 = major_write_word_or_same mh2 rem_obj next_fp in
         Seq.mem x (MH.major_objects mh3)))
  =
  let c = Seq.index mh idx in
  let hd = hd_address obj in
  let alloc_hdr =
  ...
  assert (MH.chunk_start c3 == MH.chunk_start c);
  assert (MH.chunk_end c3 == MH.chunk_end c);
  MH.read_word_in_major_lookup_index mh (hd_address x) old_hdr;
  let xidx = MH.lookup_chunk_index_value mh (hd_address x) in
  assert (MH.lookup_chunk_index mh (hd_address x) == Some xidx);
  assert (xidx < Seq.length mh);
  assert (MH.word_in_chunk (Seq.index mh xidx) (hd_address x));
  assert (MH.chunk_contains_addr (Seq.index mh xidx) (hd_address x));
  if xidx = idx then begin
    ...
    objects_in_chunk_from_head_split_preserves_member
      c c.base obj x requested_wz block_wz next_fp
      rem_wz_u rem_hd rem_obj;
    assert (Seq.index mh3 idx == c3);
    assert (c3.base == c.base);
    assert (MH.objects_in_chunk c3 == MH.objects_in_chunk_from c3 c.base);
    assert (Seq.mem x (MH.objects_in_chunk c3));
    MH.major_objects_member_at_index mh3 idx x
  end else begin
    MH.major_objects_member_in_lookup_chunk mh xidx x;
    assert (Seq.mem x (MH.objects_in_chunk (Seq.index mh xidx)));
    assert (Seq.index mh1 xidx == Seq.index mh xidx);
    assert (Seq.index mh2 xidx == Seq.index mh xidx);
    assert (Seq.index mh3 xidx == Seq.index mh xidx);
    assert (Seq.mem x (MH.objects_in_chunk (Seq.index mh3 xidx)));
    assert (xidx < Seq.length mh3);
    MH.major_objects_member_at_index mh3 xidx x
  end
#pop-options
...
let major_alloc_head_split_preserves_head_wosize
  ...
  | Some hdr ->
    ...
    | Some next_fp ->
      ...
      well_formed_no_prior_contains_selected_chunk mh idx rem_obj;
      indexed_chunk_write_preserves_contains_no_prior
        mh idx idx hd rem_obj alloc_hdr;
      indexed_chunk_write_preserves_contains_no_prior
        mh1 idx idx rem_hd rem_obj rem_hdr;
      indexed_chunk_write_preserves_contains_no_prior
        mh2 idx idx rem_obj rem_obj next_fp;
      MH.read_word_in_major_at_index mh3 rem_obj idx;
...
#push-options "--z3rlimit 20 --split_queries always --fuel 0 --ifuel 0"
let major_alloc_head_split_link_not_self
  (mh: MH.major_heap) (fp: U64.t)
  (requested_wz fuel: nat)
  : Lemma
      (requires fuel > 1 /\
               ...
               major_fl_above_zero mh fp fuel /\
               major_fl_blocks_fit mh fp fuel /\
               major_fl_head_wosize mh fp >= requested_wz + 2)
      (ensures
       (let r = major_alloc_spec_with_fuel mh fp requested_wz fuel in
        r.major_obj_out == fp /\
        r.major_fp_out <> 0UL /\
        major_alloc_result_fp_link_not_self r))
  =
  major_fl_above_zero_current mh fp fuel;
  assert (U64.v fp >= U64.v zero_addr + U64.v mword);
  let obj : obj_addr = fp in
  ...
  | Some hdr ->
    ...
    | Some next_fp ->
      ...
      assert (fuel - 1 > 0);
      major_fl_valid_next mh fp fuel;
      major_fl_above_zero_next mh obj fuel next_fp;
      assert (major_fl_valid mh next_fp (fuel - 1));
      assert (major_fl_above_zero mh next_fp (fuel - 1));
      if next_fp = 0UL then ()
      else begin
       major_fl_above_zero_current mh next_fp (fuel - 1);
       major_fl_valid_gives_mem mh next_fp (fuel - 1);
       let next_obj : obj_addr = next_fp in
       assert (Seq.mem next_obj (MH.major_objects mh));
       if next_obj = rem_obj then begin
         f_address_spec rem_hd;
         assert (f_address rem_hd == rem_obj);
         hd_f_roundtrip rem_hd;
         assert (hd_address next_obj == rem_hd);
         assert (MH.chunk_contains_addr c (hd_address next_obj));
         MH.major_objects_member_in_lookup_chunk mh idx next_obj;
         assert (Seq.mem next_obj (MH.objects_in_chunk c));
         major_fl_valid_gives_mem mh fp fuel;
         assert (Seq.mem obj (MH.major_objects mh));
         MH.major_objects_member_in_lookup_chunk mh idx obj;
         assert (Seq.mem obj (MH.objects_in_chunk c));
         assert (U64.v obj < U64.v next_obj);
         MH.objects_in_chunk_separated c obj next_obj;
         assert (MH.object_wosize_in_chunk c obj == block_wz);
         assert (U64.v obj == U64.v hd + U64.v mword);
         assert (U64.v rem_obj == U64.v hd + (requested_wz + 2) * 8);
         FStar.Math.Lemmas.distributivity_add_left
           (requested_wz + 1) 1 8;
         assert ((requested_wz + 1) * 8 + 8 ==
                 (requested_wz + 2) * 8);
         FStar.Math.Lemmas.paren_add_right
           (U64.v hd) 8 ((requested_wz + 1) * 8);
         assert (U64.v rem_obj ==
                 U64.v obj + (requested_wz + 1) * U64.v mword);
         assert (requested_wz + 1 < block_wz);
         FStar.Math.Lemmas.lemma_mult_lt_right
      ...
      assert (major_alloc_result_fp_link_not_self r)
#pop-options

#push-options "--z3rlimit 20 --split_queries always --fuel 0 --ifuel 0"
let major_alloc_head_split_remainder_not_old_object
  (mh: MH.major_heap) (fp: U64.t) (old: obj_addr)
  (requested_wz fuel: nat)
  : Lemma
      (requires fuel > 1 /\
               fp <> 0UL /\
  ...
  | Some hdr ->
    ...
    | Some next_fp ->
      ...
      assert (r.major_fp_out == rem_obj);
      assert (r.major_obj_out == fp);
      if old = rem_obj then begin
        f_address_spec rem_hd;
        assert (f_address rem_hd == rem_obj);
        hd_f_roundtrip rem_hd;
        assert (hd_address old == rem_hd);
        assert (MH.chunk_contains_addr c (hd_address old));
        MH.major_objects_member_in_lookup_chunk mh idx old;
        assert (Seq.mem old (MH.objects_in_chunk c));
        major_fl_valid_gives_mem mh fp fuel;
        assert (Seq.mem obj (MH.major_objects mh));
        MH.major_objects_member_in_lookup_chunk mh idx obj;
        assert (Seq.mem obj (MH.objects_in_chunk c));
        assert (MH.object_wosize_in_chunk c obj == block_wz);
        head_split_remainder_not_old_in_chunk
          c obj rem_obj requested_wz block_wz;
        assert False
      end
#pop-options
