...
#push-options "--z3rlimit 10 --fuel 0 --ifuel 0 --split_queries always"
let chunked_set_object_color_preserves_field_read
    (mh: MH.major_heap)
    (obj target: obj_addr)
    (color: Header.color_sem)
    (i: U64.t{U64.v i >= 1})
  ...
  | Some target_hdr ->
    SweepDefs.chunked_wosize_of_object_some mh target target_hdr;
    assert (U64.v i <= U64.v (Obj.getWosize target_hdr));
    assert (U64.v field_addr ==
            U64.v (hd_address target) + U64.v mword * U64.v i);
    SweepDefs.chunked_read_header_step mh obj;
    match SweepDefs.chunked_read_header mh obj with
...
#push-options "--z3rlimit 10 --fuel 0 --ifuel 0 --split_queries always"
let chunked_set_object_color_preserves_field_read_back
    (mh: MH.major_heap)
    (obj target: obj_addr)
    (color: Header.color_sem)
    (i: U64.t{U64.v i >= 1})
...
let chunked_make_gray_preserves_field_read
    (mh: MH.major_heap)
    (obj target: obj_addr)
    (i: U64.t{U64.v i >= 1})
    (field_addr: hp_addr)
  ...
  =
  MarkDefs.chunked_make_gray_step mh obj;
  chunked_set_object_color_preserves_field_read
    mh obj target Header.Gray i field_addr old

let chunked_make_gray_preserves_field_read_back
    (mh: MH.major_heap)
    (obj target: obj_addr)
    (i: U64.t{U64.v i >= 1})
    (field_addr: hp_addr)
  ...
  =
  MarkDefs.chunked_make_gray_step mh obj;
  chunked_set_object_color_preserves_field_read_back
    mh obj target Header.Gray i field_addr old
...
#push-options "--z3rlimit 10 --fuel 0 --ifuel 0 --split_queries always"
let chunked_set_object_color_preserves_tag_of_object
    (mh: MH.major_heap)
    (obj target: obj_addr)
    (color: Header.color_sem)
  : Lemma
...
let chunked_make_gray_preserves_tag_of_object
    (mh: MH.major_heap)
    (obj target: obj_addr)
  : Lemma
      (requires
  ...
  =
  MarkDefs.chunked_make_gray_step mh obj;
  chunked_set_object_color_preserves_tag_of_object mh obj target Header.Gray

let chunked_make_black_preserves_tag_of_object
    (mh: MH.major_heap)
    (obj target: obj_addr)
  : Lemma
      (requires
  ...
  =
  MarkDefs.chunked_make_black_step mh obj;
  chunked_set_object_color_preserves_tag_of_object mh obj target Header.Black

let chunked_make_gray_preserves_infix_status
    (mh: MH.major_heap)
    (obj target: obj_addr)
  : Lemma
      (requires
  ...
  =
  chunked_make_gray_preserves_tag_of_object mh obj target;
  SweepDefs.chunked_is_infix_step mh target;
  SweepDefs.chunked_is_infix_step (MarkDefs.chunked_make_gray mh obj) target

let chunked_make_black_preserves_infix_status
    (mh: MH.major_heap)
    (obj target: obj_addr)
  : Lemma
      (requires
      ...
      (ensures
        ...
        SweepDefs.chunked_is_infix mh target)
  =
  chunked_make_black_preserves_tag_of_object mh obj target;
  SweepDefs.chunked_is_infix_step mh target;
  SweepDefs.chunked_is_infix_step (MarkDefs.chunked_make_black mh obj) target
#pop-options
