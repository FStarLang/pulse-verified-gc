...
/// This is a key lemma but requires careful reasoning about graph well-formedness
/// For now we admit it to focus on the main DFS proof structure
#push-options "--fuel 2 --ifuel 1 --z3rlimit 50"
...
/// ---------------------------------------------------------------------------
/// Reachable Set API - Forest-Based
/// ---------------------------------------------------------------------------

/// Compute reachable set from roots with ghost forest witness
...
/// Key property: reachable_set membership is equivalent to reachable_from_set
...
let reachable_successor_closed g roots x y =
  ...
  // x is in result, so x is in forest (by property 1)
  assert (mem_forest x (reveal forest));
  // forest is successor-closed (by property 2)
  assert (forest_successor_closed g (reveal forest));
  // Therefore y is in forest
  assert (mem_forest y (reveal forest));
  // And hence y is in result
  assert (Seq.mem y result)
